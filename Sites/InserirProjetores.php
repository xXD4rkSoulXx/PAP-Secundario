...
<?php
	...
	{
		$_SESSION["pagina"]="InserirProjetores.php"; //Variável que indica que página eu pretendia entrar para assim que fizer o login redirecionar-me de volta para esta página
	...
	if(isset($_POST["selectvista"])) //Se mandei alterar a vista então:
	{
		...
		header("Location: InserirProjetores.php"); //Recarrega a página para a vista
	}
	...
	//Ideia de redirecionar-me para a página que queria sem sucessos
	
	$_SESSION["pagina"]="InserirProjetores.php"; //Variável que indica que página eu estou para que assim que entre numa página que não contenha permissão redirecione-me de volta a esta página aonde estava antes
	...
	{
		//Se não fizer essa verificação quando recarregar a página iria gerar o erro de duplicação e ia duplicar 2 ou + vezes o mesmo projetor
		if(!isset($_SESSION["registado"])) //Se ainda não cliquei no botão para registar:
		{
			...
			{
				if(isset($_FILES["btnfotoinserirescondido"]["name"])) //Se o nome da fotografia do projetor existe, significa que a fotografia do projetor existe então:
				{
					...
					echo "<script>" .
							 ...
							 "{" .
								 "document.getElementById('tipo_projetor').value='" . $tipo_projetor . "';" . //A caixa de seleção do tipo de projetor volta a ter o que estava lá selecionado
					...
					if($tipo_projetor=="Escolher") //Se o tipo do projetor não foi selecionado vai aparecer uma mensagem de erro:
					{
						echo "<script>" .
								 "window.addEventListener('load', function()" .
								 "{" .
									 "Validacao('tipo_projetor', 'O tipo do projetor não foi selecionado.');" . //Usa a função acima que criei indicando o campo com erro (tipo_projetor) e a mensagem de erro (O tipo do projetor não foi selecionado)
								 "});" .
							 "</script>";
					...
					else //Se não:
					{
						...
						if(!$bd) //Se a base de dados não existe ou ocorreu um erro:
						{
							...
							header("Location: InserirProjetores.php"); //Recarrega a página passando o erro para abrir o modal de erro
						}
					...
					if(!$erro) //Se não houve nenhum erro nos campos anteriores então:
					{
						...
						$inserir_foto_pasta=link($foto, $diretorio_novo); //Mete a fotografia do projetor na pasta de imagens dos equipamentos
						if($inserir_foto_pasta) //Se a fotografia foi inserida com sucesso na pasta então:
						{	//Insere na base de dados os dados relativos ao projetor			
							$inserir_projetor=mysqli_query($conexao, "INSERT INTO projetores VALUES(NULL, '" . $tipo_projetor . "', '" . $fabricante . "', '" . $modelo . "');");
							if($inserir_projetor) //Se foi inserido com sucesso então:
							{
								$consultar_projetor=mysqli_query($conexao, "SELECT * FROM projetores;"); //Vai buscar os dados do projetor para puder buscar o id do projetor que foi inserido à pouco
								if($consultar_projetor) //Se foi consultado com sucesso então:
								{
									$linhas_projetor=mysqli_num_rows($consultar_projetor); //Vai buscar o número de projetores que existe para depois fazer uma repetição até chegar ao último projetor inserido e buscar o id
									$idprojetor=0; //Reseta a variável com o valor 0
									
									for($i=0; $i<$linhas_projetor; $i++) //Vai repetir o número de vezes da quantidade de projetor, por exemplo, se houver 3 projetores vai repetir 3 vezes
									{
										$dados_projetor=mysqli_fetch_array($consultar_projetor); //Vai buscar os dados dos projetores à base de dados
										$idprojetor=$dados_projetor["idprojetor"]; //Vai buscar o id do projetor até chegar ao último projetor inserido e buscar o id dele
									}
									
									//Insere na base de dados os dados os dados relativos ao tipo de equipamento daí que foi necessário ter buscado à base de dados o id do projetor para puder indicar que projetor inserir, quando o id é 1 significa que não existe, como não quero inserir nem um computador, nem um monitor, nem um projetor eles todos levam o valor de 1 e indico só o id do projetor que é o equipamento que quero inserir 
									$inserir_tipoequipamento=mysqli_query($conexao, "INSERT INTO tipo_equipamentos VALUES(NULL, 1, 1, " . $idprojetor . ", 1);");
										...
										if($consultar_tipoequipamento) //Se foi consultado com sucesso então:
										{
											...
											if($inserir_equipamento) //Se foi inserido com sucesso então:
											{
												echo "<script>" .
														 "window.addEventListener('load', function()" .
														 "{" .
															 "Modal('certo', 'Projetor inserido com sucesso.');" . //Abre o modal de certo para informar ao utilizador que foi inserido com sucesso
														 "});" .
													 "</script>";
											}
											else //Se não inseriu o projetor, significa que ocorreu algum erro na base de dados
											{
								...
								{
									$_SESSION["mensagemerro"]="Erro ao consultar o id do projetor, por favor informe o administrador."; //Passa a mensagem de erro
									header("Location: InserirProjetores.php"); //Recarrega a página passando o erro para abrir o modal de erro
								}
							}
							else //Se não foi inserido na base de dados os dados relativos ao projetor, significa que ocorreu algum erro
							{
								$_SESSION["mensagemerro"]="Erro ao inserir o projetor, por favor informe o administrador."; //Passa a mensagem de erro
								header("Location: InserirProjetores.php"); //Recarrega a página passando o erro para abrir o modal de erro
							}
						}
				...
				{
					$_SESSION["mensagemerro"]="Erro ao procurar a fotografia do projetor, por favor informe o administrador."; //Passa a mensagem de erro
					header("Location: InserirProjetores.php"); //Recarrega a página passando o erro para abrir o modal de erro
				}
			}
		}
		...
		{ //Se não fizer essa verificação quando recarregar a página iria gerar o erro de duplicação e ia duplicar 2 ou + vezes o mesmo projetor
			...
			header("Location: InserirProjetores.php"); //Recarrega a página e volta para a página de Inserir Projetores
		}
	}
			...
			function Modal(tipo, mensagem)
			{
				if(!(document.getElementById("idsala")==null)) //Se a caixa de seleção idsala existir, porque pode não existir no caso de ainda não existir salas inseridos:
				{
					if(tipo!="foto") //Se o modal não é o de foto então:
					{
						...
						document.getElementById("tipo_projetor").disabled="disabled";
						...
						document.getElementById("btninserirprojetor").disabled="disabled";
						document.getElementById("btnfotoinserirmostrado").style.cursor="context-menu"; //O ponteiro do rato volta ao normal
						document.getElementById("btninserirprojetor").style.cursor="context-menu"; //O ponteiro do rato volta ao normal
					}
				}
				...
				if(!(document.getElementById("btnokinserir")==null)) //O botão inserir pode não existir por causa de ter aberto o modal com a foto e dar erro daí verificar se ele existe
				{
					...
					{
						location.href="InserirProjetores.php"; //Recarrega a página
					});
				}
																			 ...
																			 "{" +
																				 ...
																				 "{" +
																					 ...
																					 "{" +
																						 ...
																						 "document.getElementById('btninserirprojetor').style.cursor='context-menu';" +
																					 "}" +
																				 "}" +
																				...
																				"{" +
																					"location.href='InserirProjetores.php';" +
																				"});" +
	...
	<body bgcolor="#F1FAEE"> <!-- Início do body com cor de fundo a uma tonalidade de verde sem tom -->
		...
		echo "<div class='zonaequipamento'>"; //Início da zona de selecionar qual equipamento quero inserir
			...
			echo "<select name='equipamento' id='equipamento' title='Equipamento'>"; //Caixa de seleção para selecionar a qual equipamento quero inserir e redirecionar-me à página correta
				...
				echo "<option value='Projetores' selected>Projetor</option>"; //Opção por defeito por estou na página de inserir projetores
			echo "</select>";
		echo "</div>";
		echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
		echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
		echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
		echo "<h1> Inserir projetores </h1>"; //Título do formulário
		echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
		echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo 
		echo "<div class='zonainserirequipamento' id='zonainserir'>"; //Inicia a aplicação dos estilos CSS relativos à zona de inserir o formulário
			echo "<form name='frmInserir' action='#' method='POST' enctype='multipart/form-data'>"; //Início do formulário para inserir projetores, Anotar: O enctype='multipart/form-data' é obrigatório quando quero usar o input type file e enviar para a base de dados com o PHP
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<select name='tipo_projetor' id='tipo_projetor' title='Tipo de projetor'>"; //Caixa de seleção para selecionar o tipo do projetor
					echo "<option value='Escolher' selected>Selecione o tipo de projetor:</option>"; //Opção por defeito
					echo "<option value='Q'>Quadro Interativo Multimédia</option>";
					echo "<option value='V'>Videoprojetor</option>";
				echo "</select>";
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo 
				echo "<label name='errotipo_projetor' id='errotipo_projetor'></label>"; //Caso haja erro, aparecerá em baixo da caixa de seleção
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo 
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo 
				...
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<div class='zonafoto' id='zonafotoinserir'>"; //Zona de inserir a fotografia do projetor com o botão
					echo "<div class='foto' id='fotoinserir'>"; //Zona de inserir a fotografia do projetor
						echo "<div class='bordasfoto' id='bordasfotoinserir'>"; //Bordas da fotografia do projetor
							echo "<img class='tagimg' id='tagimg'>"; //Fotografia do projetor
						echo "</div>"; //Fim das bordas da fotografia do projetor
					...
					echo "</div>"; //Fim da zona de inserir a fotografia do projetor
					...
					echo "<button class='btninserir' id='btnfotoinserirmostrado'>Selecionar fotografia</button>"; //Botão mais bonito para inserir a fotografia do projetor, ele em si é bonito mas não funciona, então quando clicar faz-se depois a programação para ativar o botão escondido de inserir a fotografia
					echo "<div align='Center'><label name='errofoto' id='errofoto'></label></div>"; //Caso haja erro, aparecerá em baixo da fotografia
				echo "</div>"; //Fim da zona de inserir a fotografia do projetor com o botão
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "Prioridade: <input type='Radio' name='prioridade' id='prioridadesim' value='S' required>Sim <input type='Radio' name='prioridade' id='prioridadenao' value='N' required>Não"; //Opções sim ou não se o projetor tem prioridade para ser arranjado
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<label name='erroprioridade' id='erroprioridade'></label>"; //Caso haja erro, aparecerá em baixo da fotografia
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "Operacional: <input type='Radio' name='operacional' id='operacionalsim' value='S' required>Sim <input type='Radio' name='operacional' id='operacionalnao' value='N' required>Não"; //Opções sim ou não se o projetor está operacional, a funcionar
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<label name='errooperacional' id='errooperacional'></label>"; //Caso haja erro, aparecerá em baixo da fotografia
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<select name='idsala' id='idsala' title='Sala'>"; //Caixa de seleção para selecionar a sala a que este projetor pertence
				...
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<input type='Submit' class='btninserir' name='btninserirprojetor' id='btninserirprojetor' value='Inserir projetor'>"; //Botão para inserir projetores
