...
<?php
	...
	{
		$_SESSION["pagina"]="InserirEscolas.php"; //Variável que indica que página eu pretendia entrar para assim que fizer o login redirecionar-me de volta para esta página
	...
	if(isset($_POST["selectvista"])) //Se mandei alterar a vista então:
	{
		...
		header("Location: InserirEscolas.php"); //Recarrega a página para a vista
	}
	...
	$_SESSION["pagina"]="InserirEscolas.php"; //Variável que indica que página eu estou para que assim que entre numa página que não contenha permição redirecione-me de volta a esta página aonde estava antes
	...
	{
		//Se não fizer essa verificação quando recarregar a página iria gerar o erro de duplicação e ia duplicar 2 ou + vezes a mesma escola
		if(!isset($_SESSION["registado"])) //Se ainda não cliquei no botão para registar:
		{
			if(isset($_POST["nome_escola"])) //Se a variável nome escola existe, significa que vim via formulário e que mandei inserir, então:
			{
				...
				echo "<script>" .
						 ...
						 "{" .
							 ...
							 "document.getElementById('localizacao').value='" . $localizacao . "';" . //A caixa de texto da localização da escola volta a ter o que estava lá escrito
				...
				else //Se não:
				{
					...
					if(!$bd) //Se a base de dados não existe ou ocorreu um erro:
					{
						...
						header("Location: InserirEscolas.php"); //Recarrega a página passando o erro para abrir o modal de erro
					}
					...
					if($consultar_nome) //Se não ocorreu nenhum erro:
					{
						...
						if($linhas_nome==1) //Se retornar uma linha significa que já existe uma escola com esse nome
						{
							echo "<script>" .
									 "window.addEventListener('load', function()" .
									 "{" .
										 "Validacao('nome_escola', 'O nome da escola que introduziu já existe.');" . //Usa a função acima que criei indicando o campo com erro (nome_escola) e a mensagem de erro (O nome da escla que introduziu já existe)
									 "});" .
								 "</script>";
				...
				if($localizacao=="") //Se a localização da escola está vazia vai aparecer uma mensagem de erro:
				{
					echo "<script>" .
							 "window.addEventListener('load', function()" .
							 "{" .
								 "Validacao('localizacao', 'A localização da escola não foi preenchida.');" . //Usa a função acima que criei indicando o campo com erro (localizacao) e a mensagem de erro (A localização da escola não foi preenchido)
							 "});" .
						 "</script>";
				...
				if($idagrupamento=="Escolher") //Se o id do agrupamento não foi selecionado vai aparecer uma mensagem de erro:
				{
					echo "<script>" .
							 "window.addEventListener('load', function()" .
							 "{" .
								 "Validacao('idagrupamento', 'O agrupamento não foi selecionado.');" . //Usa a função acima que criei indicando o campo com erro (idagrupamento) e a mensagem de erro (O agrupamento não foi selecionado)
							 "});" .
						 "</script>";
				...
				if(!$erro) //Se não houve nenhum erro nos campos anteriores então:
				{
					...
					$inserir_escola=mysqli_query($conexao, "INSERT INTO escolas VALUES(NULL, '" . $nome_escola . "', '" . $localizacao . "', " . $idagrupamento . ");"); //Insere todos os campos na base de dados
					if($inserir_escola) //Se não ocorreu nenhum erro:
					{	
						...
						echo "<script>" .
								 "window.addEventListener('load', function()" .
								 "{" .
									 "Modal('certo', 'Escola inserida com sucesso.');" . //Abre o modal de certo para informar ao utilizador que foi inserido com sucesso
								 "});" .
							 "</script>";
					}
					else //Se não:
					{
						$_SESSION["mensagemerro"]="Erro ao inserir a escola, por favor informe o administrador."; //Passa a mensagem de erro
						header("Location: InserirEscolas.php"); //Recarrega a página passando o erro para abrir o modal de erro
					}
				}
			}
		...
		{ //Se não fizer essa verificação quando recarregar a página iria gerar o erro de duplicação e ia duplicar 2 ou + vezes a mesma escola
			...
			header("Location: InserirEscolas.php"); //Recarrega a página e volta para a página de Inserir Escolas
		}
	}
...
<html> <!-- Inicio do HTML -->
    <head> <!-- Início do cabeçalho -->
		...
		<script>
			...
			{	
				if(!(document.getElementById("idagrupamento")==null)) //Se a caixa de seleção idagrupamento existir, porque pode não existir no caso de ainda não existir agrupamentos inseridos:
				{
					//Quando aparecer o modal, desativa o formulário todo para impedir que haja erros de inserir duplicado
					document.getElementById("nome_escola").disabled="disabled";
					document.getElementById("localizacao").disabled="disabled";
					document.getElementById("idagrupamento").disabled="disabled";
					document.getElementById("btninserirescola").disabled="disabled";
					document.getElementById("btninserirescola").style.cursor="context-menu"; //O ponteiro do rato volta ao normal
				}
				...
				if(!(document.getElementById("btnokinserir")==null)) //Se o botão ok inserir existir, porque pode não existir no caso usar modals com 2 botões ou modals sem botão então:
				{
					...
					{
						location.href="InserirEscolas.php"; //Recarrega a página
					});
				}
	...
	<body bgcolor="#F1FAEE"> <!-- Início do body com cor de fundo a uma tonalidade de verde sem tom -->
		...
		<?php
			...
			$consultar_agrupamentos=mysqli_query($conexao, "SELECT * FROM agrupamentos ORDER BY nome_agrupamento;"); //Vai buscar todos os agrupamentos à base de dados
			if($consultar_agrupamentos) //Se não ocorreu nenhum erro:
			{
				...
				{
		echo "<h1> Inserir escola </h1>"; //Título do formulário
		...
		echo "<div class='zonainserir' id='zonainserir'>"; //Inicia a aplicação dos estilos CSS relativos à zona de inserir o formulário
			echo "<form name='frmInserir' action='#' method='POST'>"; //Início do formulário para inserir escolas
				echo "<input type='Text' name='nome_escola' id='nome_escola' placeholder='Introduza o nome da escola' title='Nome da escola' maxlength='120' required>"; //Caixa de texto para inserir o nome da escola com o máximo de 120 caracteres e obrigatório o preenchimento
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<label name='erronome_escola' id='erronome_escola'></label>"; //Caso haja erro, aparecerá em baixo da caixa de texto
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<input type='Text' name='localizacao' id='localizacao' placeholder='Introduza a localização da escola' title='Localização da escola' maxlength='120' required>"; //Caixa de texto para inserir a localização da escola com o máximo de 120 caracteres e obrigatório o preenchimento
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<label name='errolocalizacao' id='errolocalizacao'></label>"; //Caso haja erro, aparecerá em baixo da caixa de texto
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<select name='idagrupamento' id='idagrupamento' title='Agrupamento'>"; //Caixa de seleção para selecionar o agrupamento a que esta escola pertence
					echo "<option value='Escolher' selected>Selecione o agrupamento:</option>"; //Opção por defeito
					
					for($i=0; $i<$linhas_agrupamentos; $i++) //Vai repetir o número de agrupamentos, por exemplo, se houver 2 agrupamentos vai repetir 2 vezes
					{
						$dados_agrupamentos=mysqli_fetch_array($consultar_agrupamentos); //Vai buscar os dados do agrupamento
						echo "<option value='" . $dados_agrupamentos["idagrupamento"] . "'>" . $dados_agrupamentos["nome_agrupamento"] . "</option>"; //Cria as opções com os nomes dos agrupamentos existentes
					}
				
				echo "</select>";
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<label name='erroidagrupamento' id='erroidagrupamento'></label>"; //Caso haja erro, aparecerá em baixo da caixa de seleção
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<input type='Submit' class='btninserir' name='btninserirescola' id='btninserirescola' value='Inserir escola'>"; //Botão para inserir escola
			echo "</form>"; //Fim do formulário para inserir escolas
				}
