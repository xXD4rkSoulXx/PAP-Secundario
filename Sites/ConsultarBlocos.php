...
<?php
	...
	{
		$_SESSION["pagina"]="ConsultarBlocos.php"; //Variável que indica que página eu pretendia entrar para assim que fizer o login redirecionar-me de volta para esta página
	...
	if(isset($_POST["selectvista"])) //Se mandei alterar a vista então:
	{
		...
		header("Location: ConsultarBlocos.php"); //Recarrega a página para a vista
	}
	...
	$_SESSION["pagina"]="ConsultarBlocos.php"; //Variável que indica que página eu estou para que assim que entre numa página que não contenha permissão redirecione-me de volta a esta página aonde estava antes
	...
	if(!isset($_SESSION["mensagemerro"])) //Se não houver mensagem de erro:
	{
		...
		if(!isset($_SESSION["editado_eliminado"])) //Se ainda não cliquei no botão para editar ou eliminar:
		{
			if(isset($_GET["acao"])) //Se a variável acao existe, significa que cliquei ou no botão para editar ou no botão para eliminar
			{
				if($_GET["acao"]=="editar") //Se cliquei no botão para editar então:
				{
					if(isset($_GET["id"])) //Verifico se o id foi passado para não gerar erros
					{
						if(isset($_SESSION["vista"])) //Se a variável sessão existe então:
						{
							...
							{
								header("Location: ConsultarBlocos.php"); //Recarrega a página e bloqueia a pesmissão para puder editar dados
							}
						}
						else //Se não:
						{
						...
						if(!$bd) //Se a base de dados não existe ou ocorreu um erro:
						{
							...
							header("Location: ConsultarBlocos.php"); //Recarrega a página passando o erro para abrir o modal de erro
						}
						
						$consultar_bloco=mysqli_query($conexao, "SELECT * FROM escolas e, blocos b WHERE e.idescola=b.idescola AND idbloco=" . $_GET["id"] . ";"); //Vai buscar o bloco que quero editar
						if($consultar_bloco) //Se não ocorreu nenhum erro:
						{
							...
							{
								$dados_bloco=mysqli_fetch_array($consultar_bloco); //Vai buscar os dados do bloco que quero editar
								...
								if(isset($_GET["pesquisa"]) AND isset($_GET["campo"])) //Se estava uma pesquisa específica
								{
									if(isset($_GET["ordenadopor"])) //Se estava ordenado
									{
										$codigobtncancelar="document.getElementById('btncancelar').addEventListener('click', function(e)" . //Quando clicar no botão cancelar:
														   "{" .
														       "e.preventDefault();" . //Diz para não enviar nada via formulário para evitar editar o bloco pois o que quer-se é cancelar e não editar
								...
								else //Se não:
								{
									...
									else //Se não
									{
										$codigobtncancelar="document.getElementById('btncancelar').addEventListener('click', function(e)" . //Quando clicar no botão cancelar:
														   "{" .
															   "e.preventDefault();" . //Diz para não enviar nada via formulário para evitar editar o bloco pois o que quer-se é cancelar e não editar
															   "location.href='ConsultarBlocos.php';" . //Manda de volta para a página que estava
														   "});";
									}
								}
								
								$consultar_escola=mysqli_query($conexao, "SELECT * FROM escolas;"); //Vai buscar as escolas
								if($consultar_escola) //Se não ocorreu nenhum erro:
								{
									...
									{
										$selectescola="<select name=\'idescola\' class=\'dadotabela\' id=\'idescola\' title=\'Preselecionado: " . $dados_bloco["nome_escola"] . "\' style=\'width: 300px;\'>"; //A variável select vai receber o código HTML do select
										
										for($i=0; $i<$linhas_escola; $i++) /* Vai repetir o número de escolas, por exemplo, se houver 10 escolas vai repetir 10 vezes */
										{
											$dados_escola=mysqli_fetch_array($consultar_escola); //Vai buscar os dados das escolas
											$selectescola.="<option value=\'" . $dados_escola["idescola"] . "\'>" . $dados_escola["nome_escola"] . "</option>"; //Aqui repete as opções até ser todas as escolas existentes
										}
													 ...
													 "document.getElementById('zonaconsultar').innerHTML='' +" . //A zona consultar vai receber o formulário para editar
																										"'<div class=\'zonatabela\' id=\'zonatabela\'>' +" . //Início da zona da tabela
																											"'<form name=\'frmEditar\' action=\'#\' method=\'POST\'>' +" . //Início do formulário para editar
																												"'<table class=\'tabela\' style=\'width: 1050px;\'>' +" . //Início da tabela
																													...
																													"'<tbody>' +" . //Início da zona dos dados da tabela
																														"'<tr>' +" . //Início da linha da tabela
																															"'<td class=\'campotabela\'>' +" .
																																"'<label name=\'idblocomostrado\'>" . $dados_bloco["idbloco"] . "</label>' +" . //Aqui conterá o id do bloco visível porém não permite passar via formulário sendo necessário criar a caixa de texto a baixo para puder passar via formulário
																																"'<input type=\'Text\' name=\'idblocoescondidoalterar\' class=\'dadotabela\' id=\'idblocoescondidoalterar\' value=\'" . $dados_bloco["idbloco"] . "\' style=\'display: none;\'>' +" . //Aqui conterá o id do bloco numa caixa de texto para puder mandar via formulário para depois puder fazer a verificação se o id do bloco existe para verificar se mandei editar
																															"'</td>' +" .
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'nome_bloco\' class=\'dadotabela\' id=\'nome_bloco\' maxlength=\'120\' value=\'" . $dados_bloco["nome_bloco"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto do nome para puder editá-lo
																															"'<td class=\'campotabela\'>" . $selectescola . "</td>' +" . //Caixa de seleção da escola do bloco para puder editá-lo
										...
										echo "<script>" .
												 "window.addEventListener('load', function()" .
												 "{" .
													 "document.getElementById('idescola').value='" . $dados_bloco["idescola"] . "';" . //A caixa de seleção do idescola fica com o que já estava selecionado
												 "});" .
											 "</script>";
									}
				...
				{
					if(!(isset($_POST["idblocoescondidoeliminar"]))) //Se ainda não confirmei que quero eliminar:
					{
						if(isset($_GET["id"])) //Verifico se o id foi passado para não gerar erros
						{
							if(isset($_SESSION["vista"])) //Se a variável sessão existe então:
							...
							$consultar_bloco=mysqli_query($conexao, "SELECT * FROM blocos WHERE idbloco=" . $_GET["id"] . ";"); //Vai buscar os blocos que quero eliminar
							if($consultar_bloco) //Se não ocorreu nenhum erro:
							{
								$linhas_bloco=mysqli_num_rows($consultar_bloco); //Obtém o número de blocos com esse id
								if($linhas_bloco>0) //Se retornar mais que uma linha, significa que existe pelo menos um bloco então:
									...
									if(isset($_GET["pesquisa"]) AND isset($_GET["campo"])) //Se estava uma pesquisa específica
									{
										if(isset($_GET["ordenadopor"])) //Se estava ordenado
										{
											$codigobtnnao="document.getElementById('btnnao').addEventListener('click', function(e)" . //Quando clicar no botão não:
														  "{" .
														      "e.preventDefault();" . //Diz para não enviar nada via formulário para evitar eliminar o bloco pois o que quer-se é cancelar e não eliminar
															  "location.href='?pesquisa=" . $_GET["pesquisa"] . "&campo=" . $_GET["campo"] . "&ordenadopor=" . $_GET["ordenadopor"] . "';" . //Manda de volta para a página que estava com o que estava pesquisado e com o que estava ordenado
														  "});";
										}
										else //Se não
											 ...
											 "{" .
												 ...
												 "Modal('aviso', '' +" .
																...
																"'<form name=\'frmCertezaEliminar\' action=\'#\' method=\'POST\'>' +" .
																	"'<input type=\'Text\' name=\'idblocoescondidoeliminar\' class=\'dadotabela\' id=\'idblocoescondidoeliminar\' value=\'" . $_GET["id"] . "\' style=\'display: none;\'>' +" . //Mete o id do bloco escondido para depois poder verificar se mandei mesmo eliminar
																	"'<input type=\'Submit\' name=\'btnsimescondido\' id=\'btnsimescondido\' style=\'display: None;\'>' +" . //Botão do formulário escondido porque haverá outro visível com um visual melhor para mandar eliminar o bloco
																"'</form>'" .
												 ");" .
								...
								elseif($linhas_bloco==0) //Se não se as linhas são 0, significa que não existe ninguém com esse id então:
									{
			...
			if(isset($_POST["idblocoescondidoalterar"])) //Se mandei alterar os dados e já confirmei então:
			{
				//Retira os espaços em branco (trim) de trás e da frente, por exemplo, "          Dado          " passa para "Dado"
				$idbloco=trim($_GET["id"]); //Esta variável é enviada por via GET em vez de ser enviada por via POST por proteção de dados e para evitar erros
				...
				else
				{
					...
					$consultar_nome=mysqli_query($conexao, "SELECT * FROM blocos WHERE nome_bloco='" . $nome_bloco . "' AND idescola='" . $idescola . "';"); //Verifica se já existe algum bloco com essa escola com este nome
					if($consultar_nome) //Se não ocorreu nenhum erro:
					{
						...
						if($linhas_nome==1) //Se retornar uma linha significa que já existe um blocos com essa escola esse nome
						{
							$consultar_nome_antigo=mysqli_query($conexao, "SELECT * FROM blocos WHERE idbloco=" . $idbloco . " AND idescola=" . $idescola . ";"); //Vai buscar o nome antigo à base de dados
							if($consultar_nome_antigo) //Se não ocorreu nenhum erro:
							{
								$linhas_nome_antigo=mysqli_num_rows($consultar_nome_antigo); //Obtém o número de blocos com essa escola com esse nome
								if($linhas_nome_antigo==1) //Se retornar uma linha significa que a pesquisa foi bem feita
								{
									$dados_nome=mysqli_fetch_array($consultar_nome); //Recebe os dados do bloco com essa escola com esse nome
									$dados_nome_antigo=mysqli_fetch_array($consultar_nome_antigo); //Recebe os dados do bloco com essa escola que mandei editar
									
									if($dados_nome["nome_bloco"]!=$dados_nome_antigo["nome_bloco"]) //Se o nome que digitei existe e não é o nome antigo que tinha então:
									{
										echo "<script>" .
												 "window.addEventListener('load', function()" .
												 "{" .
													 "Validacao('nome_bloco', 'O nome do bloco com essa sala que introduziu já existe.');" . //Usa a função acima que criei indicando o campo com erro (nome_bloco) e a mensagem de erro (O nome do bloco que introduziu já existe)
												 "});" .
											 "</script>";
								...
								elseif($linhas_nome_antigo==0) //Se não se retornar 0 linhas, significa que houve algum erro a procurar o nome antigo então:
									...
									elseif($linhas_nome_antigo>1) //Se não se retornar mais que uma linha, significa que existe 2 blocos com essa escola com esse nome um escola não pode ter um nome que já existe então:
										{
							...
							{
								$_SESSION["mensagemerro"]="Erro ao consultar se o nome da bloco antigo com essa escola já existe, por favor informe o administrador."; //Passa a mensagem de erro
								header("Location: ConsultarBlocos.php"); //Recarrega a página passando o erro para abrir o modal de erro
							}
						}
						elseif($linhas_nome>1) //Se não se retornar mais que uma linha, significa que existe 2 blocos com essa escola com esse nome na base de dados que não se pode repetir então:
							{
								$_SESSION["mensagemerro"]="Erro. O nome do bloco com essa escola que introduziu existe mais que uma vez na base de dados, por favor informe o administrador."; //Passa a mensagem de erro
								header("Location: ConsultarBlocos.php");  //Recarrega a página passando o erro para abrir o modal de erro
							}
					...
					{
						$_SESSION["mensagemerro"]="Erro ao consultar se o nome do bloco com essa escola já existe, por favor informe o administrador."; //Passa a mensagem de erro
						header("Location: ConsultarBlocos.php"); //Recarrega a página passando o erro para abrir o modal de erro
					}
				}
				...
				if(!$erro) //Se não houve nenhum erro nos campos anteriores então:
				{
					...
					$editar_nome_bloco=mysqli_query($conexao, "UPDATE blocos SET nome_bloco='" . $nome_bloco . "' WHERE idbloco=" . $idbloco . ";"); //Edita o campo nome do bloco
					if($editar_nome_bloco) //Se não ocorreu nenhum erro:
					{	
						$editar_escola=mysqli_query($conexao, "UPDATE blocos SET idescola='" . $idescola . "' WHERE idbloco=" . $idbloco . ";"); //Edita o campo escola
						if($editar_escola) //Se não ocorreu nenhum erro:
						{	
						...
						{
							$_SESSION["mensagemerro"]="Erro ao alterar a escola, por favor informe o administrador."; //Passa a mensagem de erro
							header("Location: ConsultarBlocos.php"); //Recarrega a página passando o erro para abrir o modal de erro
						}
					}
					else //Se não:
					{
						$_SESSION["mensagemerro"]="Erro ao alterar o nome do bloco, por favor informe o administrador."; //Passa a mensagem de erro
						header("Location: ConsultarBlocos.php"); //Recarrega a página passando o erro para abrir o modal de erro
					}
				}
			}
			
			if(isset($_POST["idblocoescondidoeliminar"])) //Se mandei eliminar e selecionei a opção sim então:
			{
				$idbloco=$_GET["id"]; //Recebe o id do bloco que quero eliminar
				...
				$eliminar_bloco=mysqli_query($conexao, "DELETE FROM blocos WHERE idbloco=" . $idbloco . ";"); //Elimina o bloco
				if($eliminar_bloco) //Se o bloco foi eliminado com sucesso
				{
					$_SESSION["editado_eliminado"]=true; //A variável editado_eliminado fica verdadeiro para assim não fazer a verificação para recarregar a página e não houver erro de duplicação de dados
					...
					echo "<script>" .
							 "window.addEventListener('load', function()" .
							 "{" .
								 "Modal('certo', 'Bloco eliminado com sucesso.');" . //Abre o modal de certo para informar ao utilizador que foi eliminado com sucesso
							 "});" .
						 "</script>";
				}
				else //Se não:
				{
					$_SESSION["mensagemerro"]="Erro ao eliminar o bloco, por favor informe o administrador."; //Passa a mensagem de erro
					header("Location: ConsultarBlocos.php"); //Recarrega a página passando o erro para abrir o modal de erro
				}
			}
		}
		...
		{ //Se não fizer essa verificação quando recarregar a página iria gerar o erros
			...
			header("Location: ConsultarBlocos.php"); //Recarrega a página e volta para a página de Consultar Blocos
		}
	}
		...
		<script>
			...
			{	
				...
				if(!(document.getElementById("btnokconsultar")==null)) //Se o botão ok consultar existir, porque pode não existir no caso usar modals com 2 botões ou modals sem botão então:
				{
					...
					{
						location.href="ConsultarBlocos.php"; //Recarrega a página
					});
				}
			...
			if(isset($_GET["pesquisa"]) AND isset($_GET["campo"])) //Se foi feita uma pesquisa específica então:
			{
				if(isset($_GET["ordenadopor"])) //Se foi feito uma ordenação então:
				{
					...
					$javascriptpesquisa="<script>" .
										    "window.addEventListener('load', function()" .
										    "{" .
												"if(!(document.getElementById('btnpesquisar')==null))" . //Se a botão pesquisar existe porque pode não existir quando eu mando editar algum bloco então:
												"{" .
		...
		<div class="zonaconsultar" id="zonaconsultar"> <!-- Início da zona de consultar -->
			<?php
				...
				if((isset($_GET["pesquisa"])) AND (isset($_GET["campo"]))) //Se foi mandada uma pesquisa específica então:
				{
					if($_GET["campo"]!="O_campo_que_digitou_e_invalido") //Se o campo que foi digitado não é inválido então:
					{
						if(isset($_GET["ordenadopor"])) //Se a ordenação existe então:
						{
							...
							{
								//Pesquisa na base de dados dos blocos da pesquisa específica com a ordenação
								$consultar_blocos=mysqli_query($conexao, "SELECT * FROM escolas e, blocos b WHERE e.idescola=b.idescola AND " . $_GET["campo"] . " LIKE '%" . $_GET["pesquisa"] . "%' ORDER BY " . $_GET["ordenadopor"] . ";");
							...
							{
								//Pesquisa na base de dados dos blocos da pesquisa específica
								$consultar_blocos=mysqli_query($conexao, "SELECT * FROM escolas e, blocos b WHERE e.idescola=b.idescola AND " . $_GET["campo"] . " LIKE '%" . $_GET["pesquisa"] . "%' ORDER BY idbloco;");
								...
								$preselecionado="<script>" .
													...
													"{" .
														...
														"{" .
															"document.getElementById('pesquisar').value='" . $_GET["pesquisa"] . "';" .
															"document.getElementById('selectpesquisar').value='" . $_GET["campo"] . "';" .
															"document.getElementById('selectordenar').value='idbloco';" .
														"}" .
													"});" .
												"</script>";
							}
					...
					else //Se não:
					{
						if(isset($_GET["ordenadopor"])) //Se a ordenação existe então:
						{
							if($_GET["ordenadopor"]!="A_ordenacao_que_digitou_e_invalida") //Se a ordenação que foi digitado não é inválida então:
							{
								...
								$consultar_blocos=mysqli_query($conexao, "SELECT * FROM escolas e, blocos b WHERE e.idescola=b.idescola ORDER BY " . $_GET["ordenadopor"] . ";");
								...
								$preselecionado="<script>" .
													...
													"{" .
														"if(!(document.getElementById('btnpesquisar')==null))" . //Se a botão pesquisar existe porque pode não existir quando eu mando editar algum bloco então:
														"{" .
															"document.getElementById('selectpesquisar').value='idbloco';" .
															"document.getElementById('selectordenar').value='" . $_GET["ordenadopor"] . "';" .
														"}" .
													"});" .
												"</script>";
							...
							{
								//Pesquisa na base de dados dos blocos
								$consultar_blocos=mysqli_query($conexao, "SELECT * FROM escolas e, blocos b WHERE e.idescola=b.idescola ORDER BY idbloco;");
				...
				if($consultar_blocos) //Se não ocorreu nenhum erro:
				{
					...
					if($linhas_blocos>0) //Se retornar mais que uma linha, significa que existe pelo menos um bloco então:
					{
			...
			echo "<select name='selectpesquisar' class='selectpesquisar' id='selectpesquisar'>"; //Caixa de seleção da pesquisa específica
				echo "<option value='idbloco'>Id</option>";
			...
			echo "<select name='selectordenar' class='selectordenar' id='selectordenar'>"; //Caixa de seleção de ordenação
				echo "<option value='idbloco'>Id (Crescente)</option>";
				echo "<option value='idbloco DESC'>Id (Decrescente)</option>";
				...
				echo "<table class='tabela' id='tabela'>"; //Início da tabela
					echo "<thead>"; //Início do cabeçalho da tabela
						...
						if(isset($_SESSION["vista"])) //Se a variável de sessão existe significa que é o tipo de utilizador é um administrador então:
						{
							...
							echo "<th class='campoultimo'>Escola</th>"; //Título Escola
								}
						}
					...
					echo "<tbody>"; //Início dos campos da tabela
						for($i=0; $i<$linhas_blocos; $i++) //Vai repetir o número de blocos, por exemplo, se existir 10 blocos repete 10 vezes
						{
							$dados_blocos=mysqli_fetch_array($consultar_blocos); //Recebe os dados de cada bloco
							...
							if(isset($_GET["pesquisa"]) AND isset($_GET["campo"])) //Se já havia uma pesquisa específica então:
							{
								...
								{
									$linkeditar="?acao=editar&id=" . $dados_blocos["idbloco"] . "&pesquisa=" . $_GET["pesquisa"] . "&campo=" . $_GET["campo"] ."&ordenadopor=" . $_GET["ordenadopor"];
									$linkeliminar="?acao=eliminar&id=" . $dados_blocos["idbloco"] . "&pesquisa=" . $_GET["pesquisa"] . "&campo=" . $_GET["campo"] ."&ordenadopor=" . $_GET["ordenadopor"];
								}
								else //Se não:
								{
									$linkeditar="?acao=editar&id=" . $dados_blocos["idbloco"] . "&pesquisa=" . $_GET["pesquisa"] . "&campo=" . $_GET["campo"];
									$linkeliminar="?acao=eliminar&id=" . $dados_blocos["idbloco"] . "&pesquisa=" . $_GET["pesquisa"] . "&campo=" . $_GET["campo"];
								}
							}
							else //Se não:
							{
								...
								{
									$linkeditar="?acao=editar&id=" . $dados_blocos["idbloco"] . "&ordenadopor=" . $_GET["ordenadopor"];
									$linkeliminar="?acao=eliminar&id=" . $dados_blocos["idbloco"] . "&ordenadopor=" . $_GET["ordenadopor"];
								}
								else  //Se não:
								{
									$linkeditar="?acao=editar&id=" . $dados_blocos["idbloco"];
									$linkeliminar="?acao=eliminar&id=" . $dados_blocos["idbloco"];
								}
							}
							
						echo "<tr>"; //Início da linha da tabela
							echo "<td>" . $dados_blocos["idbloco"] . "</td>" ; //Aqui vai aparecer o id do bloco
							echo "<td>" . $dados_blocos["nome_bloco"] . "</td>"; //Aqui vai aparecer o nome do bloco
							echo "<td><a href='ConsultarEscolas.php?pesquisa=" . $dados_blocos["idescola"] . "&campo=idescola' style='color: Black;'>" . $dados_blocos["nome_escola"] . "</a></td>"; //Aqui vai aparecer o nome da escola e se clicar ele redirecionar-me-à para a página de consultar essa escola em específico
					...
					elseif($linhas_blocos==0) //Se não se retornar 0 linhas, significa que não existe nenhum bloco preenchida então:
						{
										 ...
										 "{" .
											 "document.getElementById('zonaconsultar').innerHTML='' +" .
																								...
																								"'<select name=\'selectpesquisar\' class=\'selectpesquisar\' id=\'selectpesquisar\'>' +" .
																									"'<option value=\'idbloco\'>Id</option>' +" .
																								...
																								"'<select name=\'selectordenar\' class=\'selectordenar\' id=\'selectordenar\'>' +" .
																									"'<option value=\'idbloco\'>Id (Crescente)</option>' +" .
																									"'<option value=\'idbloco DESC\'>Id (Decrescente)</option>' +" .
											 ...
											 "{" .
												 "location.href='ConsultarBlocos.php';" . //Volta para a página original
											 "});" .
										 "});" .
									 "</script>";
							...
							else //Se não:
							{
								if(isset($_SESSION["vista"])) //Se a variável vista existe
								{
									if($_SESSION["vista"]=="A") //Se a vista é de administrador então pode ser redirecionado para a página de inserir blocos:
									{
										echo "<script>" .
												 "window.addEventListener('load', function()" .
												 "{" .
									...
									else //Se não, como o utilizador não está na vista de administrador então manda para a página inicial então:
									{
										echo "<script>" .
												 ...
												 "{" .
													 ...
													 "Modal('escola', 'Sem blocos inseridos, por favor informe o administrador.');" . //Abre o Modal de que ainda não foi inserido nada
								...
								{
									if($_SESSION["tipo_utilizador"]=="A") //Se o tipo de utilizador é de administrador então pode ser redirecionado para a página de inserir blocos:
									{
										echo "<script>" .
												 "window.addEventListener('load', function()" .
												 "{" .
				...
				else //Se não, se ocorreu um erro:
				{
					if((isset($_GET["pesquisa"])) AND (isset($_GET["campo"]))) //Se foi feita uma pesquisa específica:
					{
						if(isset($_GET["ordenadopor"])) //Se foi feita uma ordenação então:
						{
							...
							if(($_GET["campo"]!="idbloco") AND ($_GET["campo"]!="nome_bloco") AND ($_GET["campo"]!="nome_escola"))
							{
								//se a ordenação digitada não for nenhum válido
								if(($_GET["ordenadopor"]!="idbloco") AND ($_GET["ordenadopor"]!="idbloco DESC") AND ($_GET["ordenadopor"]!="nome_bloco") AND ($_GET["ordenadopor"]!="nome_bloco DESC") AND ($_GET["ordenadopor"]!="nome_escola") AND ($_GET["ordenadopor"]!="nome_escola DESC"))
								{
