...
<?php
	...
	{
		$_SESSION["pagina"]="InserirQIMs.php"; //Variável que indica que página eu pretendia entrar para assim que fizer o login redirecionar-me de volta para esta página
	...
	if(isset($_POST["selectvista"])) //Se mandei alterar a vista então:
	{
		...
		header("Location: InserirQIMs.php"); //Recarrega a página para a vista
	}
	...
	$_SESSION["pagina"]="InserirQIMs.php"; //Variável que indica que página eu estou para que assim que entre numa página que não contenha permição redirecione-me de volta a esta página aonde estava antes
	...
	{
		//Se não fizer essa verificação quando recarregar a página iria gerar o erro de duplicação e ia duplicar 2 ou + vezes o mesmo quadro interativo multimédia
		if(!isset($_SESSION["registado"])) //Se ainda não cliquei no botão para registar:
		{
			...
			{
				if(isset($_FILES["btnfotoinserirescondido"]["name"])) //Se o nome da fotografia do quadro interativo multimédia existe, significa que a fotografia do quadro interativo multimédia existe então:
				{
					...
					echo "<script>" .
							 ...
							 "{" .
								 ...
								 "if('" . $prioridade . "'=='S')" . //Se o quadro interativo multimédia tinha perioridade então:
								 "{" .
									 "document.getElementById('prioridadesim').click();" . //Volta a selecionar a opção sim
								 "}" .
								 "else if('" . $prioridade . "'=='N')" . //Se não se o quadro interativo multimédia não tinha perioridade então:
									 "{" .
					...
					else //Se não:
					{
						...
						if(!$bd) //Se a base de dados não existe ou ocorreu um erro:
						{
							...
							header("Location: InserirQIMs.php"); //Recarrega a página passando o erro para abrir o modal de erro
						}
					...
					if(!$erro) //Se não houve nenhum erro nos campos anteriores então:
					{
						...
						$inserir_foto_pasta=link($foto, $diretorio_novo); //Mete a fotografia do quadro interativo multimédia na pasta de imagens dos equipamentos
						if($inserir_foto_pasta) //Se a fotografia foi inserida com sucesso na pasta então:
						{	//Insere na base de dados os dados relativos ao quadro interativo multimédia			
							$inserir_qim=mysqli_query($conexao, "INSERT INTO qims VALUES(NULL, '" . $fabricante . "', '" . $modelo . "');");
							if($inserir_qim) //Se foi inserido com sucesso então:
							{
								$consultar_qim=mysqli_query($conexao, "SELECT * FROM qims;"); //Vai buscar os dados do quadro interativo multimédia para puder buscar o id do quadro interativo multimédia que foi inserido à pouco
								if($consultar_qim) //Se foi consultado com sucesso então:
								{
									$linhas_qim=mysqli_num_rows($consultar_qim); //Vai buscar o número de quadro interativo multimédia que existe para depois fazer uma repetição até chegar ao último quadro interativo multimédia inserido e buscar o id
									$idqim=0; //Reseta a variável com o valor 0
									
									for($i=0; $i<$linhas_qim; $i++) //Vai repetir o número de vezes da quantidade de quadro interativo multimédia, por exemplo, se houver 3 quadros interativos multimédia vai repetir 3 vezes
									{
										$dados_qim=mysqli_fetch_array($consultar_qim); //Vai buscar os dados dos quadros interativos multimédia à base de dados
										$idqim=$dados_qim["idqim"]; //Vai buscar o id do quadro interativo multimédia até chegar ao último quadro interativo multimédia inserido e buscar o id dele
									}
									
									//Insere na base de dados os dados os dados relativos ao tipo de equipamento daí que foi necessário ter buscado à base de dados o id do quadro interativo multimédia para puder indicar que quadro interativo multimédia inserir, quando o id é 1 significa que não existe, como não quero inserir nem um computador, nem um monitor, nem um projetor eles todos levam o valor de 1 e indico só o id do quadro interativo multimédia que é o equipamento que quero inserir 
									$inserir_tipoequipamento=mysqli_query($conexao, "INSERT INTO tipo_equipamentos VALUES(NULL, 1, 1, 1, " . $idqim . ");");
										...
										if($consultar_tipoequipamento) //Se foi consultado com sucesso então:
										{
											...
											if($inserir_equipamento) //Se foi inserido com sucesso então:
											{
												echo "<script>" .
														 "window.addEventListener('load', function()" .
														 "{" .
															 "Modal('certo', 'Quadro interativo multimédia inserido com sucesso.');" . //Abre o modal de certo para informar ao utilizador que foi inserido com sucesso
														 "});" .
													 "</script>";
											}
											else //Se não inseriu o quadro interativo multimédia, significa que ocorreu algum erro na base de dados
											{
								...
								{
									$_SESSION["mensagemerro"]="Erro ao consultar o id do quadro interativo multimédia, por favor informe o administrador."; //Passa a mensagem de erro
									header("Location: InserirQIMs.php"); //Recarrega a página passando o erro para abrir o modal de erro
								}
							}
							else //Se não foi inserido na base de dados os dados relativos ao quadro interativo multimédia, significa que ocorreu algum erro
							{
								$_SESSION["mensagemerro"]="Erro ao inserir o quadro interativo multimédia, por favor informe o administrador."; //Passa a mensagem de erro
								header("Location: InserirQIMs.php"); //Recarrega a página passando o erro para abrir o modal de erro
							}
						}
				...
				{
					$_SESSION["mensagemerro"]="Erro ao procurar a fotografia do quadro interativo multimédia, por favor informe o administrador."; //Passa a mensagem de erro
					header("Location: InserirQIMs.php"); //Recarrega a página passando o erro para abrir o modal de erro
				}
			}
		}
		...
		{ //Se não fizer essa verificação quando recarregar a página iria gerar o erro de duplicação e ia duplicar 2 ou + vezes o mesmo quadro interativo multimédia
			...
			header("Location: InserirQIMs.php"); //Recarrega a página e volta para a página de Inserir Quadro Interativo Multimédia
		}
	}
			...
			function Modal(tipo, mensagem)
			{
				if(!(document.getElementById("idsala")==null)) //Se a caixa de seleção idsala existir, porque pode não existir no caso de ainda não existir salas inseridos:
				{
					if(tipo!="foto") //Se o modal não é o de foto então:
					{
						...
						document.getElementById("btninserirqim").disabled="disabled";
						document.getElementById("btnfotoinserirmostrado").style.cursor="context-menu"; //O ponteiro do rato volta ao normal
						document.getElementById("btninserirqim").style.cursor="context-menu"; //O ponteiro do rato volta ao normal
					}
				}
				...
				if(!(document.getElementById("btnokinserir")==null)) //O botão inserir pode não existir por causa de ter aberto o modal com a foto e dar erro daí verificar se ele existe
				{
					...
					{
						location.href="InserirQIMs.php"; //Recarrega a página
					});
				}
																			 ...
																			 "{" +
																				 ...
																				 "{" +
																					 ...
																					 "{" +
																						 ...
																						 "document.getElementById('btninserirqim').style.cursor='context-menu';" +
																					 "}" +
																				 "}" +
	...
	<body bgcolor="#F1FAEE"> <!-- Início do body com cor de fundo a uma tonalidade de verde sem tom -->
		...
		echo "<div class='zonaequipamento'>"; //Início da zona de selecionar qual equipamento quero inserir
			...
			echo "<select name='equipamento' id='equipamento' title='Equipamento'>"; //Caixa de seleção para selecionar a qual equipamento quero inserir e redirecionar-me à página correta
				echo "<option value='QIMs' selected>Quadro interativo multimédia</option>"; //Opção por defeito por estou na página de inserir quadro interativo multimédia
		...
		echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
		echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
		echo "<h1> Inserir quadro interativo multimédia </h1>"; //Título do formulário
		echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
		echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo 
		echo "<div class='zonainserirequipamento' id='zonainserir'>"; //Inicia a aplicação dos estilos CSS relativos à zona de inserir o formulário
			echo "<form name='frmInserir' action='#' method='POST' enctype='multipart/form-data'>"; //Início do formulário para inserir quadros interativos multimédia, Anotar: O enctype='multipart/form-data' é obrigatório quando quero usar o input type file e enviar para a base de dados com o PHP
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				...
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<div class='zonafoto' id='zonafotoinserir'>"; //Zona de inserir a fotografia do quadro interativo multimédia com o botão
					echo "<div class='foto' id='fotoinserir'>"; //Zona de inserir a fotografia do quadro interativo multimédia
						echo "<div class='bordasfoto' id='bordasfotoinserir'>"; //Bordas da fotografia do quadro interativo multimédia
							echo "<img class='tagimg' id='tagimg'>"; //Fotografia do quadro interativo multimédia
						echo "</div>"; //Fim das bordas da fotografia do quadro interativo multimédia
					...
					echo "</div>"; //Fim da zona de inserir a fotografia do quadro interativo multimédia
					...
					echo "<button class='btninserir' id='btnfotoinserirmostrado'>Selecionar fotografia</button>"; //Botão mais bonito para inserir a fotografia do quadro interativo multimédia, ele em si é bonito mas não funciona, então quando clicar faz-se depois a programação para ativar o botão escondido de inserir a fotografia
					echo "<div align='Center'><label name='errofoto' id='errofoto'></label></div>"; //Caso haja erro, aparecerá em baixo da fotografia
				echo "</div>"; //Fim da zona de inserir a fotografia do quadro interativo multimédia com o botão
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "Prioridade: <input type='Radio' name='prioridade' id='prioridadesim' value='S' required>Sim <input type='Radio' name='prioridade' id='prioridadenao' value='N' required>Não"; //Opções sim ou não se o quadro interativo multimédia tem prioridade para ser arranjado
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<label name='erroprioridade' id='erroprioridade'></label>"; //Caso haja erro, aparecerá em baixo da fotografia
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "Operacional: <input type='Radio' name='operacional' id='operacionalsim' value='S' required>Sim <input type='Radio' name='operacional' id='operacionalnao' value='N' required>Não"; //Opções sim ou não se o quadro interativo multimédia está operacional, a funcionar
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<label name='errooperacional' id='errooperacional'></label>"; //Caso haja erro, aparecerá em baixo da fotografia
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<select name='idsala' id='idsala' title='Sala'>"; //Caixa de seleção para selecionar a sala a que este quadro interativo multimédia pertence
				...
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<br>"; //Dá uma quebra de linha para o resto ir para baixo
				echo "<input type='Submit' class='btninserir' name='btninserirqim' id='btninserirqim' value='Inserir quadro interativo multimédia'>"; //Botão para inserir quadros interativos multimédia
