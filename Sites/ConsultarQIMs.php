...
<?php
	...
	{
		$_SESSION["pagina"]="ConsultarQIMs.php"; //Variável que indica que página eu pretendia entrar para assim que fizer o login redirecionar-me de volta para esta página
	...
	if(isset($_POST["selectvista"])) //Se mandei alterar a vista então:
	{
		...
		header("Location: ConsultarQIMs.php"); //Recarrega a página para a vista
	}
	...
	$_SESSION["pagina"]="ConsultarQIMs.php"; //Variável que indica que página eu estou para que assim que entre numa página que não contenha permissão redirecione-me de volta a esta página aonde estava antes
	...
	if(!isset($_SESSION["mensagemerro"])) //Se não houver mensagem de erro:
	{
		...
		if(!isset($_SESSION["editado_eliminado"])) //Se ainda não cliquei no botão para editar ou eliminar:
		{
			if(isset($_GET["acao"])) //Se a variável acao existe, significa que cliquei ou no botão para editar ou no botão para eliminar
			{
				if($_GET["acao"]=="editar") //Se cliquei no botão para editar então:
				{
					if(isset($_GET["id"])) //Verifico se o id foi passado para não gerar erros
					{
						...
						if(!$bd) //Se a base de dados não existe ou ocorreu um erro:
						{
							...
							header("Location: ConsultarQIMs.php"); //Recarrega a página passando o erro para abrir o modal de erro
						}
						
						$consultar_qim=mysqli_query($conexao, "SELECT * FROM escolas es, blocos b, salas s, equipamentos eq, tipo_equipamentos tp, qims q WHERE es.idescola=b.idescola AND b.idbloco=s.idbloco AND s.idsala=eq.idsala AND eq.idtpequip=tp.idtpequip AND tp.idqim=q.idqim AND tp.idqim>1 AND eq.idequipamento=" . $_GET["id"] . ";"); //Vai buscar o quadro interativo multimédia que quero editar
						if($consultar_qim) //Se não ocorreu nenhum erro:
						{
							$linhas_qim=mysqli_num_rows($consultar_qim); //Obtém o número de quadros interativos multimédia com esse id
							if($linhas_qim>0) //Se retornar mais que uma linha, significa que existe pelo menos um quadro interativo multimédia então:
							{
								$dados_qim=mysqli_fetch_array($consultar_qim); //Vai buscar os dados do quadro interativo multimédia que quero editar
								...
								if(isset($_GET["pesquisa"]) AND isset($_GET["campo"])) //Se estava uma pesquisa específica
								{
									if(isset($_GET["ordenadopor"])) //Se estava ordenado
									{
										$codigobtncancelar="document.getElementById('btncancelar').addEventListener('click', function(e)" . //Quando clicar no botão cancelar:
														   "{" .
														       "e.preventDefault();" . //Diz para não enviar nada via formulário para evitar editar o quadro interativo multimédia pois o que quer-se é cancelar e não editar
								...
								else //Se não:
								{
									...
									else //Se não
									{
										$codigobtncancelar="document.getElementById('btncancelar').addEventListener('click', function(e)" . //Quando clicar no botão cancelar:
														   "{" .
															   "e.preventDefault();" . //Diz para não enviar nada via formulário para evitar editar o quadro interativo multimédia pois o que quer-se é cancelar e não editar
															   "location.href='ConsultarQIMs.php';" . //Manda de volta para a página que estava
														   "});";
									}
								}
								...
								if($consultar_sala) //Se não ocorreu nenhum erro:
								{
									...
									{
										$selectsala="<select name=\'idsala\' class=\'dadotabela\' id=\'idsala\' title=\'Preselecionado: " . $dados_qim["nome_escola"] . " " . $dados_qim["nome_bloco"] . $dados_qim["nome_sala"] . "\' style=\'width: 350px;\'>"; //A variável select vai receber o código HTML do select
													 ...
													 "document.getElementById('zonaconsultar').innerHTML='' +" . //A zona consultar vai receber o formulário para editar
																										"'<div class=\'zonatabela\' id=\'zonatabela\'>' +" . //Início da zona da tabela
																											"'<form name=\'frmEditar\' action=\'#\' method=\'POST\'>' +" . //Início do formulário para editar
																												"'<table class=\'tabela\' style=\'width: 2050px;\'>' +" . //Início da tabela
																													...
																													"'<tbody>' +" . //Início da zona dos dados da tabela
																														"'<tr>' +" . //Início da linha da tabela
																															"'<td class=\'campotabela\'>' +" .
																																"'<label name=\'idqimmostrado\'>" . $dados_qim["idequipamento"] . "</label>' +" . //Aqui conterá o id do equipamento visível porém não permite passar via formulário sendo necessário criar a caixa de texto a baixo para puder passar via formulário
																																"'<input type=\'Text\' name=\'idqimescondidoalterar\' class=\'dadotabela\' id=\'idqimescondidoalterar\' value=\'" . $dados_qim["idequipamento"] . "\' style=\'display: none;\'>' +" . //Aqui conterá o id do equipamento numa caixa de texto para puder mandar via formulário para depois puder fazer a verificação se o id do equipamento existe para verificar se mandei editar
																															"'</td>' +" .
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'serial_number\' class=\'dadotabela\' id=\'serial_number\' maxlength=\'120\' value=\'" . $dados_qim["serial_number"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto do número de série para puder editá-lo
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'posto\' class=\'dadotabela\' id=\'posto\' maxlength=\'120\' value=\'" . $dados_qim["posto"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto do posto para puder editá-lo
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'fabricante\' class=\'dadotabela\' id=\'fabricante\' maxlength=\'120\' value=\'" . $dados_qim["fabricante"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto do fabricante para puder editá-lo
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'modelo\' class=\'dadotabela\' id=\'modelo\' maxlength=\'120\' value=\'" . $dados_qim["modelo"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto do modelo para puder editá-lo
																															"'<td class=\'campotabela\'>' +" .
																															...
																															"'</td>' +" .
																															"'<td class=\'campotabela\'>" . $selectsala . "</td>' +" . //Caixa de seleção da escola do bloco da sala do quadro interativo multimédia para puder editá-lo
																										...
																										"'<div align=\'Center\' style=\'font-size: Josefin Sans;\'>' +" . //Centra a zona da fotografia
																											...
																											"'<form name=\'frmEditarFotografia\' action=\'#\' method=\'POST\' enctype=\'multipart/form-data\'>' +" . //Início do formulário para atualizar a fotografia, Anotar: O enctype='multipart/form-data' é obrigatório quando quero usar o input type file e enviar para a base de dados com o PHP
																												"'<div class=\'zonafoto\' id=\'zonafotoalterar\'>' +" . //Zona de alterar a fotografia com botão
																													...
																													"'</div>' +" . //Fim da zona de atualizar a fotografia do quadro interativo multimédia
																													...
																													"'<button class=\'btnalterar\' id=\'btnfotoalterarmostrado\'>Selecionar fotografia</button>' +" . //Botão mais bonito para atualizar a fotografia do quadro interativo multimédia, ele em si é bonito mas não funciona, então quando clicar faz-se depois a programação para ativar o botão escondido de atualizar a fotografia
																												...
																												"'</div>' +" . //Fim da zona de atualizar a fotografia do quadro interativo multimédia com o botão
																												"'<br>' +" .
																												"'<br>' +" .
																										...
																										"'</div>';" . //Fim da zona de inserir a fotografia do quadro interativo multimédia com o botão
																 ...
																 "document.getElementById('jsmodal').innerHTML='function Modal(tipo, mensagem)' +" .
																											  "'{' +" .
																												  ...
																												  "'{' +" .
																													  ...
																													  "'{' +" .
																														  "'location.href=\'ConsultarQIMs.php\';' +" .
																													  "'});' +" .
										...
										echo "<script>" .
												 "window.addEventListener('load', function()" .
												 "{" .
													 "if('" . $dados_qim["prioridade"] . "'=='S')" . //Se o quadro interativo multimédia tinha prioridade então:
													 "{" .
														 "document.getElementById('prioridadesim').click();" . //Volta a selecionar a opção sim
													 "}" .
													 "else if('" . $dados_qim["prioridade"] . "'=='N')" . //Se não se o quadro interativo multimédia não tinha prioridade então:
														  "{" .
													 ...
													 "if('" . $dados_qim["operacional"] . "'=='S')" . //Se o quadro interativo multimédia está operacional então:
													 "{" .
														 "document.getElementById('operacionalsim').click();" . //Volta a selecionar a opção sim
													 "}" .
													 "else if('" . $dados_qim["operacional"] . "'=='N')" . //Se não se o quadro interativo multimédia não está operacional então:
														  "{" .
													 ...
													 "document.getElementById('idsala').value='" . $dados_qim["idsala"] . "';" . //A caixa de seleção do idsala fica com o que já estava selecionado
												 "});" .
											 "</script>";
									}
							...
							elseif($linhas_qim==0) //Se as linhas são 0 significa que não existe ninguém com esse id então:
								{
									header("Location: ?mensagem=O_Id_que_digitou_nao_existe"); //Manda de volta para a página e aparece uma mensagem a dizer que o id que digitou não existe
								}
								elseif($linhas_qim<0) //Se não se retornar menos que zero linhas, que existe algum erro inesperado na programação então:
									{
										$_SESSION["mensagemerro"]="Erro desconhecido. O número de linhas é inferior a 0, por favor informe o administrador."; //Passa a mensagem de erro
										header("Location: ConsultarQIMs.php"); //Recarrega a página passando o erro para abrir o modal de erro
									}
						...
						else //Se não significa que ocorreu um erro ou na base de dados ou o id que passou é inválido e contém catactéres além de números:
						{
							...
							{
								$_SESSION["mensagemerro"]="Erro ao consultar os quadros interativos multimédia, por favor informe o administrador."; //Passa a mensagem de erro
								header("Location: ConsultarQIMs.php"); //Recarrega a página passando o erro para abrir o modal de erro
							}
				...
				{
					if(!(isset($_POST["idqimescondidoeliminar"]))) //Se ainda não confirmei que quero eliminar:
					{
						if(isset($_GET["id"])) //Verifico se o id foi passado para não gerar erros
						{
							if(isset($_SESSION["vista"])) //Se a variável sessão existe então:
							{
								...
								{
									header("Location: ConsultarQIMs.php"); //Recarrega a página e bloqueia a permissão para puder eliminar dados
								}
							}
							else //Se não:
							{
								...
								{
									header("Location: ConsultarQIMs.php"); //Recarrega a página e bloqueia a pesmissão para puder eliminar dados
								}
							}
							...
							$consultar_qim=mysqli_query($conexao, "SELECT * FROM escolas es, blocos b, salas s, equipamentos eq, tipo_equipamentos tp, qims q WHERE es.idescola=b.idescola AND b.idbloco=s.idbloco AND s.idsala=eq.idsala AND eq.idtpequip=tp.idtpequip AND tp.idqim=q.idqim AND tp.idqim>1 AND eq.idequipamento=" . $_GET["id"] . ";"); //Vai buscar o quadro interativo multimédia que quero eliminar
							if($consultar_qim) //Se não ocorreu nenhum erro:
							{
								$linhas_qim=mysqli_num_rows($consultar_qim); //Obtém o número de quadros interativos multimédia com esse id
								if($linhas_qim>0) //Se retornar mais que uma linha, significa que existe pelo menos um quadro interativo multimédia então:
									...
									if(isset($_GET["pesquisa"]) AND isset($_GET["campo"])) //Se estava uma pesquisa específica
									{
										if(isset($_GET["ordenadopor"])) //Se estava ordenado
										{
											$codigobtnnao="document.getElementById('btnnao').addEventListener('click', function(e)" . //Quando clicar no botão não:
														  "{" .
														      "e.preventDefault();" . //Diz para não enviar nada via formulário para evitar eliminar o quadro interativo multimédia pois o que quer-se é cancelar e não eliminar
															  "location.href='?pesquisa=" . $_GET["pesquisa"] . "&campo=" . $_GET["campo"] . "&ordenadopor=" . $_GET["ordenadopor"] . "';" . //Manda de volta para a página que estava com o que estava pesquisado e com o que estava ordenado
														  "});";
										}
										else //Se não
											 ...
											 "{" .
												 ...
												 "Modal('aviso', '' +" .
																...
																"'<form name=\'frmCertezaEliminar\' action=\'#\' method=\'POST\'>' +" .
																	"'<input type=\'Text\' name=\'idqimescondidoeliminar\' class=\'dadotabela\' id=\'idqimescondidoeliminar\' value=\'" . $_GET["id"] . "\' style=\'display: none;\'>' +" . //Mete o id do quadro interativo multimédia escondido para depois poder verificar se mandei mesmo eliminar
																	"'<input type=\'Submit\' name=\'btnsimescondido\' id=\'btnsimescondido\' style=\'display: None;\'>' +" . //Botão do formulário escondido porque haverá outro visível com um visual melhor para mandar eliminar o quadro interativo multimédia
																"'</form>'" .
												 ");" .
								...
								elseif($linhas_qim==0) //Se não se as linhas são 0, significa que não existe ninguém com esse id então:
									{
			...
			if(isset($_POST["idqimescondidoalterar"])) //Se mandei alterar os dados e já confirmei então:
			{
				...
				$consultar_qim=mysqli_query($conexao, "SELECT * FROM escolas es, blocos b, salas s, equipamentos eq, tipo_equipamentos tp, qims q WHERE es.idescola=b.idescola AND b.idbloco=s.idbloco AND s.idsala=eq.idsala AND eq.idtpequip=tp.idtpequip AND tp.idqim=q.idqim AND tp.idqim>1 AND eq.idequipamento=" . $idequipamento . ";"); //Vai buscar o quadro interativo multimédia que quero editar
				if($consultar_qim) //Se não ocorreu nenhum erro:
				{
					$linhas_qim=mysqli_num_rows($consultar_qim); //Obtém o número de quadros interativos multimédia com esse id
					if($linhas_qim>0) //Se retornar mais que uma linha, significa que existe pelo menos um quadro interativo multimédia então:
					{
						$dados_qim=mysqli_fetch_array($consultar_qim); //Vai buscar os dados do quadro interativo multimédia que quero editar
						$idqim=$dados_qim["idqim"];
					}
					elseif($linhas_qim==0) //Se as linhas são 0 significa que não existe ninguém com esse id então:
						{
							$_SESSION["mensagemerro"]="Erro a procurar o id do quadro interativo multimédia, por favor informe o administrador."; //Passa a mensagem de erro
							header("Location: ConsultarQIMs.php"); //Recarrega a página passando o erro para abrir o modal de erro
						}
						elseif($linhas_qim<0) //Se não se retornar menos que zero linhas, que existe algum erro inesperado na programação então:
							{
				...
				echo "<script>" .
						 ...
						 "{" .
							 ...
							 "if('" . $prioridade . "'=='S')" . //Se o quadro interativo multimédia tinha prioridade então:
							 "{" .
								 "document.getElementById('prioridadesim').click();" . //Volta a selecionar a opção sim
							 "}" .
							 "else if('" . $prioridade . "'=='N')" . //Se não se o quadro interativo multimédia não tinha prioridade então:
								  "{" .
								      "document.getElementById('prioridadenao').click();" . //Volta a selecionar a opção não
								  "}" .
								  "else" . //Se não se acontecer um erro inesperado:
							 ...
							 "if('" . $operacional . "'=='S')" . //Se o quadro interativo multimédia está operacional então:
							 "{" .
								 "document.getElementById('operacionalsim').click();" . //Volta a selecionar a opção sim
							 "}" .
							 "else if('" . $operacional . "'=='N')" . //Se não se o quadro interativo multimédia não está operacional então:
								  "{" .
									  "document.getElementById('operacionalnao').click();" . //Volta a selecionar a opção não
								  "}" .
								  "else" . //Se não se acontecer um erro inesperado:
				...
				if(!$erro) //Se não houve nenhum erro nos campos anteriores então:
				{
					...
					if($editar_serial_number) //Se não ocorreu nenhum erro:
					{
						...
						{	
							$editar_fabricante=mysqli_query($conexao, "UPDATE qims SET fabricante='" . $fabricante . "' WHERE idqim=" . $idqim . ";"); //Edita o campo fabricante
							if($editar_fabricante) //Se não ocorreu nenhum erro:
							{
								$editar_modelo=mysqli_query($conexao, "UPDATE qims SET modelo='" . $modelo . "' WHERE idqim=" . $idqim . ";"); //Edita o campo nome do modelo
			...
			if(isset($_POST["idqimescondidoeliminar"])) //Se mandei eliminar e selecionei a opção sim então:
			{
				...
				$consultar_equipamento=mysqli_query($conexao, "SELECT * FROM escolas es, blocos b, salas s, equipamentos eq, tipo_equipamentos tp, qims q WHERE es.idescola=b.idescola AND b.idbloco=s.idbloco AND s.idsala=eq.idsala AND eq.idtpequip=tp.idtpequip AND tp.idqim=q.idqim AND tp.idqim>1 AND idequipamento=" . $idequipamento . ";"); //Vai buscar o equipamento que quero eliminar
				if($consultar_equipamento) //Se não ocorreu nenhum erro:
				{
					...
					if($linhas_equipamento>0) //Se retornar mais que uma linha, significa que existe pelo menos um equipamento então:
					{
						...
						$idqim=$dados_equipamento["idqim"]; //Variável que vai guardar o id do quadro interativo multimédia
				...
				if($eliminar_equipamento) //Se o equipamento foi eliminado com sucesso
				{
					...
					{
						$eliminar_qim=mysqli_query($conexao, "DELETE FROM qims WHERE idqim=" . $idqim . ";"); //Elimina o quadro interativo multimédia
						if($eliminar_qim) //Se o equipamento foi eliminado com sucesso
						{
							...
							if($eliminar_foto_pasta)
							{
								...
								echo "<script>" .
										 "window.addEventListener('load', function()" .
										 "{" .
											 "Modal('certo', 'Quadro interativo multimédia eliminado com sucesso.');" . //Abre o modal de certo para informar ao utilizador que foi eliminado com sucesso
										 "});" .
									 "</script>";
							}
							else //Se não:
							{
								$_SESSION["mensagemerro"]="Erro ao eliminar a fotografia do quadro interativo multimédia da pasta, por favor informe o administrador."; //Passa a mensagem de erro
								header("Location: ConsultarQIMs.php"); //Recarrega a página passando o erro para abrir o modal de erro
							}
						}
						else //Se não:
						{
							$_SESSION["mensagemerro"]="Erro ao eliminar o quadro interativo multimédia, por favor informe o administrador."; //Passa a mensagem de erro
							header("Location: ConsultarQIMs.php"); //Recarrega a página passando o erro para abrir o modal de erro
						}
					}
					else //Se não:
			...
			if(isset($_FILES["btnfotoalterarescondido"]["name"])) //Se o nome da fotografia do quadro interativo multimédia existe, significa que a fotografia do quadro interativo multimédia existe então:
			{
				...
				echo "<script>" .
						 ...
						 "function Validacao(campo, mensagem)" .
						 "{" .
							 "if(campo=='foto')" . //Se o campo com erro é o da fotografia do quadro interativo multimédia então:
							 "{" .
		...
		{ //Se não fizer essa verificação quando recarregar a página iria gerar o erros
			...
			header("Location: ConsultarQIMs.php"); //Recarrega a página e volta para a página de Consultar QIMs
		}
	}
    ...
    <head> <!-- Início do cabeçalho -->
		...
		<style>
			.tabela
			{
				...
				width: 2421px; /* A tabela fica com 1016px de comprimento, ((185px*6 campos)+16)=1116px */
			}
		...
		<script id="jsmodal">
			...
			function Modal(tipo, mensagem)
			{
				...
				if(!(document.getElementById("btnokconsultar")==null)) //O botão consultar pode não existir por causa de ter aberto o modal com a foto e dar erro daí verificar se ele existe
				{
					...
					{
						location.href="ConsultarQIMs.php"; //Recarrega a página
					});
				}
			...
			if(isset($_GET["pesquisa"]) AND isset($_GET["campo"])) //Se foi feita uma pesquisa específica então:
			{
				if(isset($_GET["ordenadopor"])) //Se foi feito uma ordenação então:
				{
					...
					$javascriptpesquisa="<script>" .
										    "window.addEventListener('load', function()" .
										    "{" .
												"if(!(document.getElementById('btnpesquisar')==null))" . //Se a botão pesquisar existe porque pode não existir quando eu mando editar algum quadro interativo multimédia então:
												"{" .
	...
	<body bgcolor="#F1FAEE"> <!-- Início do body com cor de fundo a uma tonalidade de verde sem tom -->
		...
		<div class="zonaconsultar" id="zonaconsultar"> <!-- Início da zona de consultar -->
			<?php
				...
				if((isset($_GET["pesquisa"])) AND (isset($_GET["campo"]))) //Se foi mandada uma pesquisa específica então:
				{
					...
					if($_GET["campo"]!="O_campo_que_digitou_e_invalido") //Se o campo que foi digitado não é inválido então:
					{
						if(isset($_GET["ordenadopor"])) //Se a ordenação existe então:
						{
							...
							{
								//Pesquisa na base de dados dos quadros interativos multimédia da pesquisa específica com a ordenação
								$consultar_qims=mysqli_query($conexao, "SELECT * FROM escolas es, blocos b, salas s, equipamentos eq, tipo_equipamentos tp, qims q WHERE es.idescola=b.idescola AND b.idbloco=s.idbloco AND s.idsala=eq.idsala AND eq.idtpequip=tp.idtpequip AND tp.idqim=q.idqim AND tp.idqim>1 AND " . $_GET["campo"] . " LIKE '%" . $_GET["pesquisa"] . "%' ORDER BY " . $_GET["ordenadopor"] . ";");
							...
							{
								//Pesquisa na base de dados dos quadros interativos multimédia da pesquisa específica
								$consultar_qims=mysqli_query($conexao, "SELECT * FROM escolas es, blocos b, salas s, equipamentos eq, tipo_equipamentos tp, qims q WHERE es.idescola=b.idescola AND b.idbloco=s.idbloco AND s.idsala=eq.idsala AND eq.idtpequip=tp.idtpequip AND tp.idqim=q.idqim AND tp.idqim>1 AND " . $_GET["campo"] . " LIKE '%" . $_GET["pesquisa"] . "%' ORDER BY idequipamento;");
					...
					else //Se não:
					{
						if(isset($_GET["ordenadopor"])) //Se a ordenação existe então:
						{
							if($_GET["ordenadopor"]!="A_ordenacao_que_digitou_e_invalida") //Se a ordenação que foi digitado não é inválida então:
							{
								//Pesquisa na base de dados dos quadros interativos multimédia com a ordenação
								$consultar_qims=mysqli_query($conexao, "SELECT * FROM escolas es, blocos b, salas s, equipamentos eq, tipo_equipamentos tp, qims q WHERE es.idescola=b.idescola AND b.idbloco=s.idbloco AND s.idsala=eq.idsala AND eq.idtpequip=tp.idtpequip AND tp.idqim=q.idqim AND tp.idqim>1 ORDER BY " . $_GET["ordenadopor"] . ";");
							...
							{
								//Pesquisa na base de dados dos quadros interativos multimédia
								$consultar_qims=mysqli_query($conexao, "SELECT * FROM escolas es, blocos b, salas s, equipamentos eq, tipo_equipamentos tp, qims q WHERE es.idescola=b.idescola AND b.idbloco=s.idbloco AND s.idsala=eq.idsala AND eq.idtpequip=tp.idtpequip AND tp.idqim=q.idqim AND tp.idqim>1 ORDER BY idequipamento;");
				...
				if($consultar_qims) //Se não ocorreu nenhum erro:
				{
					$linhas_qims=mysqli_num_rows($consultar_qims); //Obtém o número de quadros interativos multimédia
					if($linhas_qims>0) //Se retornar mais que uma linha, significa que existe pelo menos um quadro interativo multimédia então:
					{
			...
			echo "<select name='equipamento' class='selectordenar' id='equipamento'>";
				echo "<option value='QIMs' selected>Quadro interativo multimédia</option>"; //Opção por defeito por estou na página de consultar quadros interativos multimédia
			...
			echo "<div class='zonatabela' id='zonatabela'>"; //Início da zona da tabela
				echo "<table class='tabela' id='tabela'>"; //Início da tabela
							...
							elseif($_SESSION["vista"]=="E") //Se não não mete os cabeçalhos e os botões e define este campo como último
								{
									echo "<script>" .
											 ...
											 "{" .
												 ...
												 "{" .
													 "document.getElementById('tabela').style.width='2050px';" . //Dimiui o tamanho da tabela
												 "}" .
											 "});" .
										 "</script>";
					...
					echo "<tbody>"; //Início dos campos da tabela
						for($i=0; $i<$linhas_qims; $i++) //Vai repetir o número de quadros interativos multimédia, por exemplo, se existir 10 quadros interativos multimédia repete 10 vezes
						{
							$dados_qims=mysqli_fetch_array($consultar_qims); //Recebe os dados de cada quadro interativo multimédia
							...
							if(isset($_GET["pesquisa"]) AND isset($_GET["campo"])) //Se já havia uma pesquisa específica então:
							{
								...
								{
									$linkeditar="?acao=editar&id=" . $dados_qims["idequipamento"] . "&pesquisa=" . $_GET["pesquisa"] . "&campo=" . $_GET["campo"] ."&ordenadopor=" . $_GET["ordenadopor"];
									$linkeliminar="?acao=eliminar&id=" . $dados_qims["idequipamento"] . "&pesquisa=" . $_GET["pesquisa"] . "&campo=" . $_GET["campo"] ."&ordenadopor=" . $_GET["ordenadopor"];
								}
								else //Se não:
								{
									$linkeditar="?acao=editar&id=" . $dados_qims["idequipamento"] . "&pesquisa=" . $_GET["pesquisa"] . "&campo=" . $_GET["campo"];
									$linkeliminar="?acao=eliminar&id=" . $dados_qims["idequipamento"] . "&pesquisa=" . $_GET["pesquisa"] . "&campo=" . $_GET["campo"];
								}
							}
							else //Se não:
							{
								...
								{
									$linkeditar="?acao=editar&id=" . $dados_qims["idequipamento"] . "&ordenadopor=" . $_GET["ordenadopor"];
									$linkeliminar="?acao=eliminar&id=" . $dados_qims["idequipamento"] . "&ordenadopor=" . $_GET["ordenadopor"];
								}
								else  //Se não:
								{
									$linkeditar="?acao=editar&id=" . $dados_qims["idequipamento"];
									$linkeliminar="?acao=eliminar&id=" . $dados_qims["idequipamento"];
								}
							}
								
						echo "<tr>"; //Início da linha da tabela
							echo "<td>" . $dados_qims["idequipamento"] . "</td>" ; //Aqui vai aparecer o id do equipamento
							echo "<td>" . $dados_qims["serial_number"] . "</td>"; //Aqui vai aparecer o número de série
							echo "<td>" . $dados_qims["posto"] . "</td>"; //Aqui vai aparecer o posto
							echo "<td>" . $dados_qims["fabricante"] . "</td>"; //Aqui vai aparecer o fabricante
							echo "<td>" . $dados_qims["modelo"] . "</td>"; //Aqui vai aparecer o modelo
							echo "<td><img src='" . $dados_qims["foto"] . "' name='foto" . ($i+1) . "' id='foto" . ($i+1) . "' width='50px' height='50px' alt='Erro ao carregar a fotografia do quadro interativo multimédia, por favor informe o administrador.' title='Clique na imagem para ver melhor.' style='cursor: Pointer;'></td>"; //Aqui vai aparecer a fotografia
							
							switch($dados_qims["prioridade"]) //Caso a prioridade:
							{
				  //Seja S:
				  ...
				  case "N": echo "<td>Não</td>"; //Aparece Não na tabela
							...
							switch($dados_qims["operacional"]) //Caso o operacional:
							{
				  //Seja S:
				  case "S": echo "<td>Sim</td>"; //Aparece Sim na tabela
							break; //Fim da opção
				  ...
				  case "N": echo "<td>Não</td>"; //Aparece Não na tabela
							...
							echo "<td><a href='ConsultarEscolas.php?pesquisa=" . $dados_qims["idescola"] . "&campo=idescola' style='color: Black;'>" . $dados_qims["nome_escola"] . "</a></td>"; //Aqui vai aparecer o nome da escola e se clicar ele redirecionar-me-à para a página de consultar essa escola em específico
							echo "<td><a href='ConsultarBlocos.php?pesquisa=" . $dados_qims["idbloco"] . "&campo=idbloco' style='color: Black;'>" . $dados_qims["nome_bloco"] . "</a></td>"; //Aqui vai aparecer o nome do bloco e se clicar ele redirecionar-me-à para a página de consultar esse bloco em específico
							echo "<td><a href='ConsultarSalas.php?pesquisa=" . $dados_qims["idsala"] . "&campo=idsala' style='color: Black;'>" . $dados_qims["nome_sala"] . "</a></td>"; //Aqui vai aparecer o nome da sala e se clicar ele redirecionar-me-à para a página de consultar essa sala em específico
							...
							for($j=21; $j<(count(str_split($dados_qims["foto"]))); $j++)
							{
								$nome_foto.=$dados_qims["foto"][$j]; //Faz a variável nome_foto receber o nome da fotografia
							}
						...
						echo "</tr>"; //Fim da linha
							...
							echo "<script>" .
									 "window.addEventListener('load', function()" .
									 "{" .
										 "if(!(document.getElementById('foto" . ($i+1) . "')==null))" . //Se a fotografia existe porque pode não existir no caso de estivar a editar os dados do quadro interativo multimédia então:
										 "{" .
											 ...
											 "{" .
												 ...
												 "document.getElementById('modalfoto').src='" . $dados_qims["foto"] . "';" . //Faz o modal receber a fotografia
			...
			echo "</div>"; //Fim da zona da tabela
					...
					elseif($linhas_qims==0) //Se não se retornar 0 linhas, significa que não existe nenhum quadro interativo multimédia preenchido então:
						{
							if(isset($_GET["pesquisa"]) AND isset($_GET["campo"])) //Se o campo pesquisa existe, significa que na verdade os dados existem mas só mandei fazer uma pesquisa específica que não retornou resultados então:
							{
								...
								echo "<script>" .
										 ...
										 "{" .
											 ...
											 "{" .
												 "location.href='ConsultarQIMs.php';" . //Volta para a página original
											 "});" .
										 "});" .
									 "</script>";
							...
							else //Se não:
							{
								if(isset($_SESSION["vista"])) //Se a variável vista existe
								{
									if($_SESSION["vista"]=="A") //Se a vista é de administrador então pode ser redirecionado para a página de inserir quadros interativos multimédia:
									{
										echo "<script>" .
												 "window.addEventListener('load', function()" .
												 "{" .
													 ...
													 "Modal('qim', 'Sem quadros interativos multimédia inseridos.');" . //Abre o Modal de que ainda não foi inserido nada
													 
													 "document.getElementById('zonabtnmodal').innerHTML='Será redirecionado à página para inserir quadros interativos multimédia dentro de ' + segundos + ' segundos.';" . //Faz os botões desaparecerem e aparece a mensagem que vai redirecionar para a página de inserir quadros interativos multimédia dentro de 5 segundos
													 ...
													 "{" .
														 "location.href='InserirQIMs.php';" . //Redireciona para a página de inserir quadros interativos multimédia
													 "}, 6000);" .
													 ...
													 "{" .
														 ...
														 "{" .
															 "document.getElementById('zonabtnmodal').innerHTML='Será redirecionado à página para inserir quadros interativos multimédia dentro de ' + segundos + ' segundo.';" .
															 "segundos--;" . //Diminui a variável segundos para ir acompanhando o tempo
														 "}" .
														 "else" .
														 "{" .
															 "document.getElementById('zonabtnmodal').innerHTML='Será redirecionado à página para inserir quadros interativos multimédia dentro de ' + segundos + ' segundos.';" .
															 "segundos--;" . //Diminui a variável segundos para ir acompanhando o tempo
														 "}" .
													 "}, 1000);" .
												 "});" .
									...
									else //Se não, como o utilizador não está na vista de administrador então manda para a página inicial então:
									{
										echo "<script>" .
												 ...
												 "{" .
													 ...
													 "Modal('qim', 'Sem quadros interativos multimédia inseridos, por favor informe o administrador.');" . //Abre o Modal de que ainda não foi inserido nada
						...
						elseif($linhas_qims<0) //Se não se retornar menos que zero linhas, que existe algum erro inesperado na programação então:
							{
								$_SESSION["mensagemerro"]="Erro desconhecido. O número de linhas é inferior a 0, por favor informe o administrador."; //Passa a mensagem de erro
								header("Location: ConsultarQIMs.php"); //Recarrega a página passando o erro para abrir o modal de erro
							}
				...
				else //Se não, se ocorreu um erro:
				{
					if((isset($_GET["pesquisa"])) AND (isset($_GET["campo"]))) //Se foi feita uma pesquisa específica:
					{
						if(isset($_GET["ordenadopor"])) //Se foi feita uma ordenação então:
						{
							...
							if(($_GET["campo"]!="idequipamento") AND ($_GET["campo"]!="serial_number") AND ($_GET["campo"]!="posto") AND ($_GET["campo"]!="fabricante") AND ($_GET["campo"]!="modelo") AND ($_GET["campo"]!="prioridade") AND ($_GET["campo"]!="operacional") AND ($_GET["campo"]!="nome_escola") AND ($_GET["campo"]!="nome_bloco") AND ($_GET["campo"]!="nome_sala"))
							{
								//se a ordenação digitada não for nenhum válido
								if(($_GET["ordenadopor"]!="idequipamento") AND ($_GET["ordenadopor"]!="idequipamento DESC") AND ($_GET["ordenadopor"]!="serial_number") AND ($_GET["ordenadopor"]!="serial_number DESC") AND ($_GET["ordenadopor"]!="posto") AND ($_GET["ordenadopor"]!="posto DESC") AND ($_GET["ordenadopor"]!="fabricante") AND ($_GET["ordenadopor"]!="fabricante DESC") AND ($_GET["ordenadopor"]!="modelo") AND ($_GET["ordenadopor"]!="modelo DESC") AND ($_GET["ordenadopor"]!="prioridade") AND ($_GET["ordenadopor"]!="prioridade DESC") AND ($_GET["ordenadopor"]!="operacional") AND ($_GET["ordenadopor"]!="operacional DESC") AND ($_GET["ordenadopor"]!="nome_escola") AND ($_GET["ordenadopor"]!="nome_escola DESC") AND ($_GET["ordenadopor"]!="nome_bloco") AND ($_GET["ordenadopor"]!="nome_bloco DESC") AND ($_GET["ordenadopor"]!="nome_sala") AND ($_GET["ordenadopor"]!="nome_sala DESC"))
								{
