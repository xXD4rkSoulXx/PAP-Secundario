...
<?php
	...
	{
		$_SESSION["pagina"]="ConsultarComputadores.php"; //Variável que indica que página eu pretendia entrar para assim que fizer o login redirecionar-me de volta para esta página
	...
	if(isset($_POST["selectvista"])) //Se mandei alterar a vista então:
	{
		...
		header("Location: ConsultarComputadores.php"); //Recarrega a página para a vista
	}
	...
	$_SESSION["pagina"]="ConsultarComputadores.php"; //Variável que indica que página eu estou para que assim que entre numa página que não contenha permissão redirecione-me de volta a esta página aonde estava antes
	...
	if(!isset($_SESSION["mensagemerro"])) //Se não houver mensagem de erro:
	{
		...
		if(!isset($_SESSION["editado_eliminado"])) //Se ainda não cliquei no botão para editar ou eliminar:
		{
			if(isset($_GET["acao"])) //Se a variável acao existe, significa que cliquei ou no botão para editar ou no botão para eliminar
			{
				if($_GET["acao"]=="editar") //Se cliquei no botão para editar então:
				{
					if(isset($_GET["id"])) //Verifico se o id foi passado para não gerar erros
					{
						...
						if(!$bd) //Se a base de dados não existe ou ocorreu um erro:
						{
							...
							header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
						}
						
						$consultar_computador=mysqli_query($conexao, "SELECT * FROM escolas es, blocos b, salas s, equipamentos eq, tipo_equipamentos tp, computadores c WHERE es.idescola=b.idescola AND b.idbloco=s.idbloco AND s.idsala=eq.idsala AND eq.idtpequip=tp.idtpequip AND tp.idcomputador=c.idcomputador AND tp.idcomputador>1 AND eq.idequipamento=" . $_GET["id"] . ";"); //Vai buscar o computador que quero editar
						if($consultar_computador) //Se não ocorreu nenhum erro:
						{
							$linhas_computador=mysqli_num_rows($consultar_computador); //Obtém o número de computadores com esse id
							if($linhas_computador>0) //Se retornar mais que uma linha, significa que existe pelo menos um computador então:
							{
								$dados_computador=mysqli_fetch_array($consultar_computador); //Vai buscar os dados do computador que quero editar
								...
								if(isset($_GET["pesquisa"]) AND isset($_GET["campo"])) //Se estava uma pesquisa específica
								{
									if(isset($_GET["ordenadopor"])) //Se estava ordenado
									{
										$codigobtncancelar="document.getElementById('btncancelar').addEventListener('click', function(e)" . //Quando clicar no botão cancelar:
														   "{" .
														       "e.preventDefault();" . //Diz para não enviar nada via formulário para evitar editar o computador pois o que quer-se é cancelar e não editar
								...
								else //Se não:
								{
									...
									else //Se não
									{
										$codigobtncancelar="document.getElementById('btncancelar').addEventListener('click', function(e)" . //Quando clicar no botão cancelar:
														   "{" .
															   "e.preventDefault();" . //Diz para não enviar nada via formulário para evitar editar o computador pois o que quer-se é cancelar e não editar
															   "location.href='ConsultarComputadores.php';" . //Manda de volta para a página que estava
														   "});";
									}
								}
								...
								if($consultar_sala) //Se não ocorreu nenhum erro:
								{
									...
									{
										$selectsala="<select name=\'idsala\' class=\'dadotabela\' id=\'idsala\' title=\'Preselecionado: " . $dados_computador["nome_escola"] . " " . $dados_computador["nome_bloco"] . $dados_computador["nome_sala"] . "\' style=\'width: 350px;\'>"; //A variável select vai receber o código HTML do select
													 ...
													 "document.getElementById('zonaconsultar').innerHTML='' +" . //A zona consultar vai receber o formulário para editar
																										"'<div class=\'zonatabela\' id=\'zonatabela\'>' +" . //Início da zona da tabela
																											"'<form name=\'frmEditar\' action=\'#\' method=\'POST\'>' +" . //Início do formulário para editar
																												"'<table class=\'tabela\' style=\'width: 4100px;\'>' +" . //Início da tabela
																													"'<thead>' +" . //Início da zona dos cabeçalhos com os títulos das colunas
																														"'<tr>' +" . //Início da linha da tabela
																															...
																															"'<th class=\'campotabela\'>Tipo de computador</th>' +" . //Título dos tipos de computador
																															...
																															"'<th class=\'campotabela\'>Modelo</th>' +" . //Título dos modelo
																															"'<th class=\'campotabela\'>Sistema Operativo</th>' +" . //Título dos sistemas operativos
																															"'<th class=\'campotabela\'>Memória RAM</th>' +" . //Título das ram's
																															"'<th class=\'campotabela\'>CPU (Processador)</th>' +" . //Título dos cpu's
																															"'<th class=\'campotabela\'>Motherboard</th>' +" . //Título das motherboards
																															"'<th class=\'campotabela\'>Disco rígido</th>' +" . //Título dos discos rígidos
																															"'<th class=\'campotabela\'>Disco ótico</th>' +" . //Título dos discos óticos
																															"'<th class=\'campotabela\'>Placa gráfica</th>' +" . //Título dos gráficas
																															"'<th class=\'campotabela\'>Placa de rede</th>' +" . //Título das placas de rede
																															"'<th class=\'campotabela\'>Fonte de alimentação</th>' +" . //Título das fontes de alimentação
																															"'<th class=\'campotabela\'>Cooler (Ventoinha)</th>' +" . //Título dos coolers
																													...
																													"'<tbody>' +" . //Início da zona dos dados da tabela
																														"'<tr>' +" . //Início da linha da tabela
																															"'<td class=\'campotabela\'>' +" .
																																"'<label name=\'idcomputadormostrado\'>" . $dados_computador["idequipamento"] . "</label>' +" . //Aqui conterá o id do equipamento visível porém não permite passar via formulário sendo necessário criar a caixa de texto a baixo para puder passar via formulário
																																"'<input type=\'Text\' name=\'idcomputadorescondidoalterar\' class=\'dadotabela\' id=\'idcomputadorescondidoalterar\' value=\'" . $dados_computador["idequipamento"] . "\' style=\'display: none;\'>' +" . //Aqui conterá o id do equipamento numa caixa de texto para puder mandar via formulário para depois puder fazer a verificação se o id do equipamento existe para verificar se mandei editar
																															"'</td>' +" .
																															"'<td class=\'campotabela\'>' +" .
																																"'<select name=\'tipo_pc\' class=\'dadotabela\' id=\'tipo_pc\'>' +" . //Caixa de seleção do tipo de computador
																																	"'<option value=\'T\'>Torre</option>' +" .
																																	"'<option value=\'P\'>Portátil</option>' +" .
																																	"'<option value=\'S\'>Servidor</option>' +" .
																																"'</select>' +" .
																															"'</td>' +" .
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'serial_number\' class=\'dadotabela\' id=\'serial_number\' maxlength=\'120\' value=\'" . $dados_computador["serial_number"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto do número de série para puder editá-lo
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'posto\' class=\'dadotabela\' id=\'posto\' maxlength=\'120\' value=\'" . $dados_computador["posto"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto do posto para puder editá-lo
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'fabricante\' class=\'dadotabela\' id=\'fabricante\' maxlength=\'120\' value=\'" . $dados_computador["fabricante"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto do fabricante para puder editá-lo
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'modelo\' class=\'dadotabela\' id=\'modelo\' maxlength=\'120\' value=\'" . $dados_computador["modelo"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto do modelo para puder editá-lo
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'so\' class=\'dadotabela\' id=\'so\' placeholder=\'Não obrigatório\' maxlength=\'120\' value=\'" . $dados_computador["so"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto do sistema opetavivo para puder editá-lo
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'memoria_ram\' class=\'dadotabela\' placeholder=\'Não obrigatório\' id=\'memoria_ram\' maxlength=\'120\' value=\'" . $dados_computador["ram"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto da ram para puder editá-lo
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'cpu\' class=\'dadotabela\' id=\'cpu\' placeholder=\'Não obrigatório\' maxlength=\'120\' value=\'" . $dados_computador["cpu"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto do cpu para puder editá-lo
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'motherboard\' class=\'dadotabela\' id=\'motherboard\' placeholder=\'Não obrigatório\' maxlength=\'120\' value=\'" . $dados_computador["motherboard"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto da motherboard para puder editá-lo
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'disco_rigido\' class=\'dadotabela\' id=\'disco_rigido\' placeholder=\'Não obrigatório\' maxlength=\'120\' value=\'" . $dados_computador["disco_rigido"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto do disco rígido para puder editá-lo
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'disco_otico\' class=\'dadotabela\' id=\'disco_otico\' placeholder=\'Não obrigatório\' maxlength=\'120\' value=\'" . $dados_computador["disco_otico"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto do disco ótico para puder editá-lo
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'placa_grafica\' class=\'dadotabela\' id=\'placa_grafica\' placeholder=\'Não obrigatório\' maxlength=\'120\' value=\'" . $dados_computador["placa_grafica"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto da gréfica para puder editá-lo
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'placa_rede\' class=\'dadotabela\' id=\'placa_rede\' placeholder=\'Não obrigatório\' maxlength=\'120\' value=\'" . $dados_computador["placa_rede"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto da placa de rede para puder editá-lo
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'fonte_alimentacao\' class=\'dadotabela\' id=\'fonte_alimentacao\' placeholder=\'Não obrigatório\' maxlength=\'120\' value=\'" . $dados_computador["fonte_alimentacao"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto da fonte de alimentação para puder editá-lo
																															"'<td class=\'campotabela\'><input type=\'Text\' name=\'cooler\' class=\'dadotabela\' id=\'cooler\' placeholder=\'Não obrigatório\' maxlength=\'120\' value=\'" . $dados_computador["cooler"] . "\' style=\'width: 120px;\'></td>' +" . //Caixa de texto do cooler para puder editá-lo
																															"'<td class=\'campotabela\'>' +" .
																															...
																															"'</td>' +" .
																															"'<td class=\'campotabela\'>" . $selectsala . "</td>' +" . //Caixa de seleção da escola do bloco da sala do computador para puder editá-lo
																										...
																										"'<div align=\'Center\' style=\'font-size: Josefin Sans;\'>' +" . //Centra a zona da fotografia
																											...
																											"'<form name=\'frmEditarFotografia\' action=\'#\' method=\'POST\' enctype=\'multipart/form-data\'>' +" . //Início do formulário para atualizar a fotografia, Anotar: O enctype='multipart/form-data' é obrigatório quando quero usar o input type file e enviar para a base de dados com o PHP
																												"'<div class=\'zonafoto\' id=\'zonafotoalterar\'>' +" . //Zona de alterar a fotografia com botão
																													...
																													"'</div>' +" . //Fim da zona de atualizar a fotografia do computador
																													...
																													"'<button class=\'btnalterar\' id=\'btnfotoalterarmostrado\'>Selecionar fotografia</button>' +" . //Botão mais bonito para atualizar a fotografia do computador, ele em si é bonito mas não funciona, então quando clicar faz-se depois a programação para ativar o botão escondido de atualizar a fotografia
																												...
																												"'</div>' +" . //Fim da zona de atualizar a fotografia do computador com o botão
																												"'<br>' +" .
																												"'<br>' +" .
																										...
																										"'</div>';" . //Fim da zona de inserir a fotografia do computador com o botão
																 ...
																 "document.getElementById('jsmodal').innerHTML='function Modal(tipo, mensagem)' +" .
																											  "'{' +" .
																												  ...
																												  "'{' +" .
																													  ...
																													  "'{' +" .
																														  "'location.href=\'ConsultarComputadores.php\';' +" .
																													  "'});' +" .
										...
										echo "<script>" .
												 "window.addEventListener('load', function()" .
												 "{" .
													 "document.getElementById('tipo_pc').value='" . $dados_computador["tipo"] . "';" . //A caixa de seleção do tipo do computador fica com o que já estava selecionado
													 
													 "if('" . $dados_computador["prioridade"] . "'=='S')" . //Se o computador tinha perioridade então:
													 "{" .
														 "document.getElementById('prioridadesim').click();" . //Volta a selecionar a opção sim
													 "}" .
													 "else if('" . $dados_computador["prioridade"] . "'=='N')" . //Se não se o computador não tinha perioridade então:
														  "{" .
													 ...
													 "if('" . $dados_computador["operacional"] . "'=='S')" . //Se o computador está operacional então:
													 "{" .
														 "document.getElementById('operacionalsim').click();" . //Volta a selecionar a opção sim
													 "}" .
													 "else if('" . $dados_computador["operacional"] . "'=='N')" . //Se não se o computador não está operacional então:
														  "{" .
													 ...
													 "document.getElementById('idsala').value='" . $dados_computador["idsala"] . "';" . //A caixa de seleção do idsala fica com o que já estava selecionado
												 "});" .
											 "</script>";
									}
							...
							elseif($linhas_computador==0) //Se as linhas são 0 significa que não existe ninguém com esse id então:
								{
									header("Location: ?mensagem=O_Id_que_digitou_nao_existe"); //Manda de volta para a página e aparece uma mensagem a dizer que o id que digitou não existe
								}
								elseif($linhas_computador<0) //Se não se retornar menos que zero linhas, que existe algum erro inesperado na programação então:
									{
										$_SESSION["mensagemerro"]="Erro desconhecido. O número de linhas é inferior a 0, por favor informe o administrador."; //Passa a mensagem de erro
										header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
									}
						...
						else //Se não significa que ocorreu um erro ou na base de dados ou o id que passou é inválido e contém catactéres além de números:
						{
							...
							{
								$_SESSION["mensagemerro"]="Erro ao consultar os computadores, por favor informe o administrador."; //Passa a mensagem de erro
								header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
							}
				...
				{
					if(!(isset($_POST["idcomputadorescondidoeliminar"]))) //Se ainda não confirmei que quero eliminar:
					{
						if(isset($_GET["id"])) //Verifico se o id foi passado para não gerar erros
						{
							if(isset($_SESSION["vista"])) //Se a variável sessão existe então:
							{
								...
								{
									header("Location: ConsultarComputadores.php"); //Recarrega a página e bloqueia a pesmissão para puder eliminar dados
								}
							}
							else //Se não:
							{
							...
							$consultar_computador=mysqli_query($conexao, "SELECT * FROM escolas es, blocos b, salas s, equipamentos eq, tipo_equipamentos tp, computadores c WHERE es.idescola=b.idescola AND b.idbloco=s.idbloco AND s.idsala=eq.idsala AND eq.idtpequip=tp.idtpequip AND tp.idcomputador=c.idcomputador AND tp.idcomputador>1 AND eq.idequipamento=" . $_GET["id"] . ";"); //Vai buscar o computador que quero eliminar
							if($consultar_computador) //Se não ocorreu nenhum erro:
							{
								$linhas_computador=mysqli_num_rows($consultar_computador); //Obtém o número de computadores com esse id
								if($linhas_computador>0) //Se retornar mais que uma linha, significa que existe pelo menos um computador então:
									...
									if(isset($_GET["pesquisa"]) AND isset($_GET["campo"])) //Se estava uma pesquisa específica
									{
										if(isset($_GET["ordenadopor"])) //Se estava ordenado
										{
											$codigobtnnao="document.getElementById('btnnao').addEventListener('click', function(e)" . //Quando clicar no botão não:
														  "{" .
														      "e.preventDefault();" . //Diz para não enviar nada via formulário para evitar eliminar o computador pois o que quer-se é cancelar e não eliminar
															  "location.href='?pesquisa=" . $_GET["pesquisa"] . "&campo=" . $_GET["campo"] . "&ordenadopor=" . $_GET["ordenadopor"] . "';" . //Manda de volta para a página que estava com o que estava pesquisado e com o que estava ordenado
														  "});";
										}
										else //Se não
											 ...
											 "{" .
												 ...
												 "Modal('aviso', '' +" .
																...
																"'<form name=\'frmCertezaEliminar\' action=\'#\' method=\'POST\'>' +" .
																	"'<input type=\'Text\' name=\'idcomputadorescondidoeliminar\' class=\'dadotabela\' id=\'idcomputadorescondidoeliminar\' value=\'" . $_GET["id"] . "\' style=\'display: none;\'>' +" . //Mete o id do computador escondido para depois poder verificar se mandei mesmo eliminar
																	"'<input type=\'Submit\' name=\'btnsimescondido\' id=\'btnsimescondido\' style=\'display: None;\'>' +" . //Botão do formulário escondido porque haverá outro visível com um visual melhor para mandar eliminar o computador
																"'</form>'" .
												 ");" .
								...
								elseif($linhas_computador==0) //Se não se as linhas são 0, significa que não existe ninguém com esse id então:
									{
			...
			if(isset($_POST["idcomputadorescondidoalterar"])) //Se mandei alterar os dados e já confirmei então:
			{
				...
				$consultar_computador=mysqli_query($conexao, "SELECT * FROM escolas es, blocos b, salas s, equipamentos eq, tipo_equipamentos tp, computadores c WHERE es.idescola=b.idescola AND b.idbloco=s.idbloco AND s.idsala=eq.idsala AND eq.idtpequip=tp.idtpequip AND tp.idcomputador=c.idcomputador AND tp.idcomputador>1 AND eq.idequipamento=" . $idequipamento . ";"); //Vai buscar o computador que quero editar
				if($consultar_computador) //Se não ocorreu nenhum erro:
				{
					$linhas_computador=mysqli_num_rows($consultar_computador); //Obtém o número de computadores com esse id
					if($linhas_computador>0) //Se retornar mais que uma linha, significa que existe pelo menos um computador então:
					{
						$dados_computador=mysqli_fetch_array($consultar_computador); //Vai buscar os dados do computador que quero editar
						$idcomputador=$dados_computador["idcomputador"];
					}
					elseif($linhas_computador==0) //Se as linhas são 0 significa que não existe ninguém com esse id então:
						{
							$_SESSION["mensagemerro"]="Erro a procurar o id do computador, por favor informe o administrador."; //Passa a mensagem de erro
							header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
						}
						elseif($linhas_computador<0) //Se não se retornar menos que zero linhas, que existe algum erro inesperado na programação então:
							{
				...
				if(!$erro) //Se não houve nenhum erro nos campos anteriores então:
				{
					...
					$editar_tipo_pc=mysqli_query($conexao, "UPDATE computadores SET tipo='" . $tipo_pc . "' WHERE idcomputador=" . $idcomputador . ";"); //Edita o campo tipo de computador
					if($editar_tipo_pc) //Se não ocorreu nenhum erro:
					{	
						...
						if($editar_serial_number) //Se não ocorreu nenhum erro:
						{
							...
							{	
								$editar_fabricante=mysqli_query($conexao, "UPDATE computadores SET fabricante='" . $fabricante . "' WHERE idcomputador=" . $idcomputador . ";"); //Edita o campo fabricante
								if($editar_fabricante) //Se não ocorreu nenhum erro:
								{
									$editar_modelo=mysqli_query($conexao, "UPDATE computadores SET modelo='" . $modelo . "' WHERE idcomputador=" . $idcomputador . ";"); //Edita o campo nome do modelo
									if($editar_modelo) //Se não ocorreu nenhum erro:
									{	
										$editar_so=mysqli_query($conexao, "UPDATE computadores SET so='" . $so . "' WHERE idcomputador=" . $idcomputador . ";"); //Edita o campo sistema operativo
										if($editar_so) //Se não ocorreu nenhum erro:
										{
											$editar_ram=mysqli_query($conexao, "UPDATE computadores SET ram='" . $memoria_ram . "' WHERE idcomputador=" . $idcomputador . ";"); //Edita o campo memoria ram
											if($editar_ram) //Se não ocorreu nenhum erro:
											{	
												$editar_cpu=mysqli_query($conexao, "UPDATE computadores SET cpu='" . $cpu . "' WHERE idcomputador=" . $idcomputador . ";"); //Edita o campo cpu
												if($editar_cpu) //Se não ocorreu nenhum erro:
												{
													$editar_motherboard=mysqli_query($conexao, "UPDATE computadores SET motherboard='" . $motherboard . "' WHERE idcomputador=" . $idcomputador . ";"); //Edita o campo motherboard
													if($editar_motherboard) //Se não ocorreu nenhum erro:
													{	
														$editar_disco_rigido=mysqli_query($conexao, "UPDATE computadores SET disco_rigido='" . $disco_rigido . "' WHERE idcomputador=" . $idcomputador . ";"); //Edita o campo disco rigido
														if($editar_disco_rigido) //Se não ocorreu nenhum erro:
														{
															$editar_disco_otico=mysqli_query($conexao, "UPDATE computadores SET disco_otico='" . $disco_otico . "' WHERE idcomputador=" . $idcomputador . ";"); //Edita o campo disco otico
															if($editar_disco_otico) //Se não ocorreu nenhum erro:
															{	
																$editar_placa_grafica=mysqli_query($conexao, "UPDATE computadores SET placa_grafica='" . $placa_grafica . "' WHERE idcomputador=" . $idcomputador . ";"); //Edita o campo placa grafica
																if($editar_placa_grafica) //Se não ocorreu nenhum erro:
																{
																	$editar_placa_rede=mysqli_query($conexao, "UPDATE computadores SET placa_rede='" . $placa_rede . "' WHERE idcomputador=" . $idcomputador . ";"); //Edita o campo placa de rede
																	if($editar_placa_rede) //Se não ocorreu nenhum erro:
																	{	
																		$editar_fonte_alimentacao=mysqli_query($conexao, "UPDATE computadores SET fonte_alimentacao='" . $fonte_alimentacao . "' WHERE idcomputador=" . $idcomputador . ";"); //Edita o campo fonte de alimentação
																		if($editar_fonte_alimentacao) //Se não ocorreu nenhum erro:
																		{
																			$editar_cooler=mysqli_query($conexao, "UPDATE computadores SET cooler='" . $cooler . "' WHERE idcomputador=" . $idcomputador . ";"); //Edita o campo cooler
																			if($editar_cooler) //Se não ocorreu nenhum erro:
																			{	
																			...
																			{
																				$_SESSION["mensagemerro"]="Erro ao alterar o cooler (ventoinha), por favor informe o administrador."; //Passa a mensagem de erro
																				header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
																			}
																		}
																		else //Se não:
																		{
																			$_SESSION["mensagemerro"]="Erro ao alterar a fonte de alimentação, por favor informe o administrador."; //Passa a mensagem de erro
																			header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
																		}
																	}
																	else //Se não:
																	{
																		$_SESSION["mensagemerro"]="Erro ao alterar a placa de rede, por favor informe o administrador."; //Passa a mensagem de erro
																		header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
																	}
																}
																else //Se não:
																{
																	$_SESSION["mensagemerro"]="Erro ao alterar a placa gráfica, por favor informe o administrador."; //Passa a mensagem de erro
																	header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
																}
															}
															else //Se não:
															{
																$_SESSION["mensagemerro"]="Erro ao alterar o disco ótico, por favor informe o administrador."; //Passa a mensagem de erro
																header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
															}
														}
														else //Se não:
														{
															$_SESSION["mensagemerro"]="Erro ao alterar o disco rígido, por favor informe o administrador."; //Passa a mensagem de erro
															header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
														}
													}
													else //Se não:
													{
														$_SESSION["mensagemerro"]="Erro ao alterar a motherboard, por favor informe o administrador."; //Passa a mensagem de erro
														header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
													}
												}
												else //Se não:
												{
													$_SESSION["mensagemerro"]="Erro ao alterar o CPU (processador), por favor informe o administrador."; //Passa a mensagem de erro
													header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
												}
											}
											else //Se não:
											{
												$_SESSION["mensagemerro"]="Erro ao alterar a memória RAM, por favor informe o administrador."; //Passa a mensagem de erro
												header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
											}
										}
										else //Se não:
										{
											$_SESSION["mensagemerro"]="Erro ao alterar o SO (sistema operativo), por favor informe o administrador."; //Passa a mensagem de erro
											header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
										}
									}
									else //Se não:
					...
					{
						$_SESSION["mensagemerro"]="Erro ao alterar o tipo de computador, por favor informe o administrador."; //Passa a mensagem de erro
						header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
					}
				}
			}
			
			if(isset($_POST["idcomputadorescondidoeliminar"])) //Se mandei eliminar e selecionei a opção sim então:
			{
				...
				$consultar_equipamento=mysqli_query($conexao, "SELECT * FROM escolas es, blocos b, salas s, equipamentos eq, tipo_equipamentos tp, computadores c WHERE es.idescola=b.idescola AND b.idbloco=s.idbloco AND s.idsala=eq.idsala AND eq.idtpequip=tp.idtpequip AND tp.idcomputador=c.idcomputador AND tp.idcomputador>1 AND idequipamento=" . $idequipamento . ";"); //Vai buscar o equipamento que quero eliminar
				if($consultar_equipamento) //Se não ocorreu nenhum erro:
				{
					...
					if($linhas_equipamento>0) //Se retornar mais que uma linha, significa que existe pelo menos um equipamento então:
					{
						...
						$idcomputador=$dados_equipamento["idcomputador"]; //Variável que vai guardar o id do tipo de equipamento
				...
				if($eliminar_equipamento) //Se o equipamento foi eliminado com sucesso
				{
					...
					{
						$eliminar_computador=mysqli_query($conexao, "DELETE FROM computadores WHERE idcomputador=" . $idcomputador . ";"); //Elimina o computador
						if($eliminar_computador) //Se o equipamento foi eliminado com sucesso
						{
							...
							if($eliminar_foto_pasta)
							{
								...
								echo "<script>" .
										 "window.addEventListener('load', function()" .
										 "{" .
											 "Modal('certo', 'Computador eliminado com sucesso.');" . //Abre o modal de certo para informar ao utilizador que foi eliminado com sucesso
										 "});" .
									 "</script>";
							}
							else //Se não:
							{
								$_SESSION["mensagemerro"]="Erro ao eliminar a fotografia do computador da pasta, por favor informe o administrador."; //Passa a mensagem de erro
								header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
							}
						}
						else //Se não:
						{
							$_SESSION["mensagemerro"]="Erro ao eliminar o computador, por favor informe o administrador."; //Passa a mensagem de erro
							header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
						}
					}
					else //Se não:
			...
			if(isset($_FILES["btnfotoalterarescondido"]["name"])) //Se o nome da fotografia do computador existe, significa que a fotografia do computador existe então:
			{
		...
		{ //Se não fizer essa verificação quando recarregar a página iria gerar o erros
			...
			header("Location: ConsultarComputadores.php"); //Recarrega a página e volta para a página de Consultar Computadores
		}
	}
    ...
    <head> <!-- Início do cabeçalho -->
		...
		<style>
			.tabela
			{
				...
				width: 4300px; /* A tabela fica com 1016px de comprimento, ((185px*6 campos)+16)=1116px */
			}
		...
		<script id="jsmodal">
			...
			function Modal(tipo, mensagem)
			{
				...
				if(!(document.getElementById("btnokconsultar")==null)) //O botão consultar pode não existir por causa de ter aberto o modal com a foto e dar erro daí verificar se ele existe
				{
					...
					{
						location.href="ConsultarComputadores.php"; //Recarrega a página
					});
				}
			...
			if(isset($_GET["pesquisa"]) AND isset($_GET["campo"])) //Se foi feita uma pesquisa específica então:
			{
				if(isset($_GET["ordenadopor"])) //Se foi feito uma ordenação então:
				{
					...
					$javascriptpesquisa="<script>" .
										    "window.addEventListener('load', function()" .
										    "{" .
												"if(!(document.getElementById('btnpesquisar')==null))" . //Se a botão pesquisar existe porque pode não existir quando eu mando editar algum computador então:
												"{" .
	...
	<body bgcolor="#F1FAEE"> <!-- Início do body com cor de fundo a uma tonalidade de verde sem tom -->
		...
		<div class="zonaconsultar" id="zonaconsultar"> <!-- Início da zona de consultar -->
			<?php
				...
				{
					//Se a pesquisa foi Torre e o campo foi tipo de computador então:
					if((($_GET["pesquisa"]=="To") OR ($_GET["pesquisa"]=="Tor") OR ($_GET["pesquisa"]=="Torr") OR ($_GET["pesquisa"]=="Torre") OR ($_GET["pesquisa"]=="Torres")) AND ($_GET["campo"]=="tipo"))
					{
						$pesquisaexata=$_GET["pesquisa"]; //A pesquisa exata recebe o que o foi pesquisado
						$_GET["pesquisa"]="T"; //Temporariamente vai ser pesquisado por T devido à base de dados
						
					}	//Se não se a pesquisa foi Portátil e o campo foi tipo de computador então:
					elseif((($_GET["pesquisa"]=="Po") OR ($_GET["pesquisa"]=="Por") OR ($_GET["pesquisa"]=="Port") OR ($_GET["pesquisa"]=="Portá") OR ($_GET["pesquisa"]=="Portát") OR ($_GET["pesquisa"]=="Portáti") OR ($_GET["pesquisa"]=="Portátil") OR ($_GET["pesquisa"]=="Portáte") OR ($_GET["pesquisa"]=="Portátei") OR ($_GET["pesquisa"]=="Portáteis")) AND ($_GET["campo"]=="tipo"))
						{
							$pesquisaexata=$_GET["pesquisa"]; //A pesquisa exata recebe o que o foi pesquisado
							$_GET["pesquisa"]="P"; //Temporariamente vai ser pesquisado por P devido à base de dados
							
						}	//Se não se a pesquisa foi Servidor e o campo foi tipo de computador então:
						elseif((($_GET["pesquisa"]=="Se") OR ($_GET["pesquisa"]=="Ser") OR ($_GET["pesquisa"]=="Serv") OR ($_GET["pesquisa"]) OR ($_GET["pesquisa"]=="Servi") OR ($_GET["pesquisa"]=="Servid") OR ($_GET["pesquisa"]=="Servido") OR ($_GET["pesquisa"]=="Servidor") OR ($_GET["pesquisa"]=="Servidore") OR ($_GET["pesquisa"]=="Servidores")) AND ($_GET["campo"]=="tipo"))
							{
								$pesquisaexata=$_GET["pesquisa"]; //A pesquisa exata recebe o que o foi pesquisado
					...
					if($_GET["campo"]!="O_campo_que_digitou_e_invalido") //Se o campo que foi digitado não é inválido então:
					{
						if(isset($_GET["ordenadopor"])) //Se a ordenação existe então:
						{
							...
							{
								//Pesquisa na base de dados dos computadores da pesquisa específica com a ordenação
								$consultar_computadores=mysqli_query($conexao, "SELECT * FROM escolas es, blocos b, salas s, equipamentos eq, tipo_equipamentos tp, computadores c WHERE es.idescola=b.idescola AND b.idbloco=s.idbloco AND s.idsala=eq.idsala AND eq.idtpequip=tp.idtpequip AND tp.idcomputador=c.idcomputador AND tp.idcomputador>1 AND " . $_GET["campo"] . " LIKE '%" . $_GET["pesquisa"] . "%' ORDER BY " . $_GET["ordenadopor"] . ";");
							...
							{
								//Pesquisa na base de dados dos computadores da pesquisa específica
								$consultar_computadores=mysqli_query($conexao, "SELECT * FROM escolas es, blocos b, salas s, equipamentos eq, tipo_equipamentos tp, computadores c WHERE es.idescola=b.idescola AND b.idbloco=s.idbloco AND s.idsala=eq.idsala AND eq.idtpequip=tp.idtpequip AND tp.idcomputador=c.idcomputador AND tp.idcomputador>1 AND " . $_GET["campo"] . " LIKE '%" . $_GET["pesquisa"] . "%' ORDER BY idequipamento;");
					...
					else //Se não:
					{
						if(isset($_GET["ordenadopor"])) //Se a ordenação existe então:
						{
							if($_GET["ordenadopor"]!="A_ordenacao_que_digitou_e_invalida") //Se a ordenação que foi digitado não é inválida então:
							{
								//Pesquisa na base de dados dos computadores com a ordenação
								$consultar_computadores=mysqli_query($conexao, "SELECT * FROM escolas es, blocos b, salas s, equipamentos eq, tipo_equipamentos tp, computadores c WHERE es.idescola=b.idescola AND b.idbloco=s.idbloco AND s.idsala=eq.idsala AND eq.idtpequip=tp.idtpequip AND tp.idcomputador=c.idcomputador AND tp.idcomputador>1 ORDER BY " . $_GET["ordenadopor"] . ";");
							...
							{
								//Pesquisa na base de dados dos computadores
								$consultar_computadores=mysqli_query($conexao, "SELECT * FROM escolas es, blocos b, salas s, equipamentos eq, tipo_equipamentos tp, computadores c WHERE es.idescola=b.idescola AND b.idbloco=s.idbloco AND s.idsala=eq.idsala AND eq.idtpequip=tp.idtpequip AND tp.idcomputador=c.idcomputador AND tp.idcomputador>1 ORDER BY idequipamento;");
				...
				if($consultar_computadores) //Se não ocorreu nenhum erro:
				{
					$linhas_computadores=mysqli_num_rows($consultar_computadores); //Obtém o número de computadores
					if($linhas_computadores>0) //Se retornar mais que uma linha, significa que existe pelo menos um computador então:
					{
			...
			echo "<select name='selectpesquisar' class='selectpesquisar' id='selectpesquisar'>"; //Caixa de seleção da pesquisa específica
				...
				echo "<option value='tipo'>Tipo de computador</option>";
				...
				echo "<option value='so'>Sistema operativo</option>";
				echo "<option value='ram'>Memória RAM</option>";
				echo "<option value='cpu'>CPU (Processador)</option>";
				echo "<option value='motherboard'>Motherboard</option>";
				echo "<option value='disco_rigido'>Disco rígido</option>";
				echo "<option value='disco_otico'>Disco ótico</option>";
				echo "<option value='placa_grafica'>Placa gráfica</option>";
				echo "<option value='placa_rede'>Placa de rede</option>";
				echo "<option value='fonte_alimentacao'>Fonte de alimentação</option>";
				echo "<option value='cooler'>Cooler (Ventoinha)</option>";
			...
			echo "<select name='selectordenar' class='selectordenar' id='selectordenar'>"; //Caixa de seleção de ordenação
				...
				echo "<option value='tipo'>Tipo de computador (Crescente)</option>";
				echo "<option value='tipo DESC'>Tipo de computador (Decrescente)</option>";
				...
				echo "<option value='so'>Sistema operativo (Crescente)</option>";
				echo "<option value='so DESC'>Sistema operativo (Decrescente)</option>";
				echo "<option value='ram'>Memória RAM (Crescente)</option>";
				echo "<option value='ram DESC'>Memória RAM</option>";
				echo "<option value='cpu'>CPU (Processador) (Crescente)</option>";
				echo "<option value='cpu DESC'>CPU (Processador) (Decrescente)</option>";
				echo "<option value='motherboard'>Motherboard (Crescente)</option>";
				echo "<option value='motherboard DESC'>Motherboard (Decrescente)</option>";
				echo "<option value='disco_rigido'>Disco rígido (Crescente)</option>";
				echo "<option value='disco_rigido DESC'>Disco rígido (Decrescente)</option>";
				echo "<option value='disco_otico'>Disco ótico (Crescente)</option>";
				echo "<option value='disco_otico DESC'>Disco ótico (Decrescente)</option>";
				echo "<option value='placa_grafica'>Placa gráfica (Crescente)</option>";
				echo "<option value='placa_grafica DESC'>Placa gráfica (Decrescente)</option>";
				echo "<option value='placa_rede'>Placa de rede (Crescente)</option>";
				echo "<option value='placa_rede DESC'>Placa de rede (Decrescente)</option>";
				echo "<option value='fonte_alimentacao'>Fonte de alimentação (Crescente)</option>";
				echo "<option value='fonte_alimentacao DESC'>Fonte de alimentação (Decrescente)</option>";
				echo "<option value='cooler'>Cooler (Ventoinha) (Crescente)</option>";
				echo "<option value='cooler DESC'>Cooler (Ventoinha) (Decrescente)</option>";
			...
			echo "<select name='equipamento' class='selectordenar' id='equipamento'>";
				...
				echo "<option value='Computadores' selected>Computador</option>"; //Opção por defeito por estou na página de consultar computadores
				...
				echo "<table class='tabela' id='tabela'>"; //Início da tabela
					echo "<thead>"; //Início do cabeçalho da tabela
						echo "<tr>"; //Início da linha da tabela
							...
							echo "<th class='campotabela'>Tipo de computador</th>"; //Título Tipo de computador
							...
							echo "<th class='campotabela'>Sistema Operativo</th>"; //Título Sistema operativo
							echo "<th class='campotabela'>Memória RAM</th>"; //Título Memória RAM
							echo "<th class='campotabela'>CPU (Processador)</th>"; //Título CPU (Processador)
							echo "<th class='campotabela'>Motherboard</th>"; //Título Motherboard
							echo "<th class='campotabela'>Disco rígido</th>"; //Título Disco rígido
							echo "<th class='campotabela'>Disco ótico</th>"; //Título Disco ótico
							echo "<th class='campotabela'>Placa gráfica</th>"; //Título Placa gráfica
							echo "<th class='campotabela'>Placa de rede</th>"; //Título Placa de ede
							echo "<th class='campotabela'>Fonte de alimentação</th>"; //Título Fonte de alimentação
							echo "<th class='campotabela'>Cooler (Ventoinha)</th>"; //Título Cooler (Ventoinha)
							...
							elseif($_SESSION["vista"]=="E") //Se não não mete os cabeçalhos e os botões e define este campo como último
								{
									echo "<script>" .
											 ...
											 "{" .
												 ...
												 "{" .
													 "document.getElementById('tabela').style.width='4200px';" . //Dimiui o tamanho da tabela
												 "}" .
											 "});" .
										 "</script>";
					...
					echo "<tbody>"; //Início dos campos da tabela
						for($i=0; $i<$linhas_computadores; $i++) //Vai repetir o número de computadores, por exemplo, se existir 10 computadores repete 10 vezes
						{
							$dados_computadores=mysqli_fetch_array($consultar_computadores); //Recebe os dados de cada computador
							...
							if(isset($_GET["pesquisa"]) AND isset($_GET["campo"])) //Se já havia uma pesquisa específica então:
							{
								...
								{
									$linkeditar="?acao=editar&id=" . $dados_computadores["idequipamento"] . "&pesquisa=" . $_GET["pesquisa"] . "&campo=" . $_GET["campo"] ."&ordenadopor=" . $_GET["ordenadopor"];
									$linkeliminar="?acao=eliminar&id=" . $dados_computadores["idequipamento"] . "&pesquisa=" . $_GET["pesquisa"] . "&campo=" . $_GET["campo"] ."&ordenadopor=" . $_GET["ordenadopor"];
								}
								else //Se não:
								{
									$linkeditar="?acao=editar&id=" . $dados_computadores["idequipamento"] . "&pesquisa=" . $_GET["pesquisa"] . "&campo=" . $_GET["campo"];
									$linkeliminar="?acao=eliminar&id=" . $dados_computadores["idequipamento"] . "&pesquisa=" . $_GET["pesquisa"] . "&campo=" . $_GET["campo"];
								}
							}
							else //Se não:
							{
								...
								{
									$linkeditar="?acao=editar&id=" . $dados_computadores["idequipamento"] . "&ordenadopor=" . $_GET["ordenadopor"];
									$linkeliminar="?acao=eliminar&id=" . $dados_computadores["idequipamento"] . "&ordenadopor=" . $_GET["ordenadopor"];
								}
								else  //Se não:
								{
									$linkeditar="?acao=editar&id=" . $dados_computadores["idequipamento"];
									$linkeliminar="?acao=eliminar&id=" . $dados_computadores["idequipamento"];
								}
							}
								
						echo "<tr>"; //Início da linha da tabela
							echo "<td>" . $dados_computadores["idequipamento"] . "</td>" ; //Aqui vai aparecer o id do equipamento
							
							switch($dados_computadores["tipo"]) //Caso o tipo de computador:
							{
				  //Seja T:
				  case "T": echo "<td>Torre</td>"; //Aparece Torre na tabela
							break; //Fim da opção
							
				  //Seja P
				  case "P": echo "<td>Portátil</td>"; //Aparece Portátil na tabela
							break; //Fim da opção
							
				  //Seja S
				  case "S": echo "<td>Servidor</td>"; //Aparece Servidor na tabela
							break; //Fim da opção
							
								default: $_SESSION["mensagemerro"]="O tipo de computador é inválido, por favor informe o administrador."; //Passa a mensagem de erro
										 header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro;
							}
							
							echo "<td>" . $dados_computadores["serial_number"] . "</td>"; //Aqui vai aparecer o número de série
							echo "<td>" . $dados_computadores["posto"] . "</td>"; //Aqui vai aparecer o posto
							echo "<td>" . $dados_computadores["fabricante"] . "</td>"; //Aqui vai aparecer o fabricante
							echo "<td>" . $dados_computadores["modelo"] . "</td>"; //Aqui vai aparecer o modelo
							
							if($dados_computadores["so"]==null)
							{
							echo "<td>Sem sistema operativo preenchido.</td>"; //Aqui vai avisar que não existe sistema operativo preenchido
							}
							else
							{
							echo "<td>" . $dados_computadores["so"] . "</td>"; //Aqui vai aparecer o sistema operativo
							}
							
							if($dados_computadores["ram"]==null)
							{
							echo "<td>Sem memória RAM preenchida.</td>"; //Aqui vai avisar que não existe memória RAM preenchida
							}
							else
							{
							echo "<td>" . $dados_computadores["ram"] . "</td>"; //Aqui vai aparecer a memória RAM
							}
							
							if($dados_computadores["cpu"]==null)
							{
							echo "<td>Sem CPU (Processador) preenchido.</td>"; //Aqui vai avisar que não existe CPU (Processador) preenchido
							}
							else
							{
							echo "<td>" . $dados_computadores["cpu"] . "</td>"; //Aqui vai aparecer o CPU (Processador)
							}
							
							if($dados_computadores["motherboard"]==null)
							{
							echo "<td>Sem motherboard preenchida.</td>"; //Aqui vai avisar que não existe motherboard preenchida
							}
							else
							{
							echo "<td>" . $dados_computadores["motherboard"] . "</td>"; //Aqui vai aparecer a motherboard
							}
							
							if($dados_computadores["disco_rigido"]==null)
							{
							echo "<td>Sem disco rígido preenchido.</td>"; //Aqui vai avisar que não existe disco rígido preenchido
							}
							else
							{
							echo "<td>" . $dados_computadores["disco_rigido"] . "</td>"; //Aqui vai aparecer o disco rígido
							}
							
							if($dados_computadores["disco_otico"]==null)
							{
							echo "<td>Sem disco ótico preenchido.</td>"; //Aqui vai avisar que não existe disco ótico preenchido
							}
							else
							{
							echo "<td>" . $dados_computadores["disco_otico"] . "</td>"; //Aqui vai aparecer o disco ótico
							}
							
							if($dados_computadores["placa_grafica"]==null)
							{
							echo "<td>Sem placa gráfica preenchida.</td>"; //Aqui vai avisar que não existe placa gráfica preenchida
							}
							else
							{
							echo "<td>" . $dados_computadores["placa_grafica"] . "</td>"; //Aqui vai aparecer a placa gráfica
							}
							
							if($dados_computadores["placa_rede"]==null)
							{
							echo "<td>Sem placa de rede preenchida.</td>"; //Aqui vai avisar que não existe placa de rede preenchida
							}
							else
							{
							echo "<td>" . $dados_computadores["placa_rede"] . "</td>"; //Aqui vai aparecer a placa de rede
							}
							
							if($dados_computadores["fonte_alimentacao"]==null)
							{
							echo "<td>Sem fonte de alimentação preenchido.</td>"; //Aqui vai avisar que não existe fonte de alimentação preenchida
							}
							else
							{
							echo "<td>" . $dados_computadores["fonte_alimentacao"] . "</td>"; //Aqui vai aparecer a fonte de alimentação
							}
							
							if($dados_computadores["cooler"]==null)
							{
							echo "<td>Sem cooler (ventoinha) preenchido.</td>"; //Aqui vai avisar que não existe cooler (ventoinha) preenchido
							}
							else
							{
							echo "<td>" . $dados_computadores["cooler"] . "</td>"; //Aqui vai aparecer o cooler (ventoinha)
							}
							
							echo "<td><img src='" . $dados_computadores["foto"] . "' name='foto" . ($i+1) . "' id='foto" . ($i+1) . "' width='50px' height='50px' alt='Erro ao carregar a fotografia do computador, por favor informe o administrador.' title='Clique na imagem para ver melhor.' style='cursor: Pointer;'></td>"; //Aqui vai aparecer a fotografia
							
							switch($dados_computadores["prioridade"]) //Caso a prioridade:
							{
				  //Seja S:
				  ...
				  case "N": echo "<td>Não</td>"; //Aparece Não na tabela
							...
							switch($dados_computadores["operacional"]) //Caso o operacional:
							{
				  //Seja S:
				  case "S": echo "<td>Sim</td>"; //Aparece Sim na tabela
							break; //Fim da opção
				  ...
				  case "N": echo "<td>Não</td>"; //Aparece Não na tabela
							...
							echo "<td><a href='ConsultarEscolas.php?pesquisa=" . $dados_computadores["idescola"] . "&campo=idescola' style='color: Black;'>" . $dados_computadores["nome_escola"] . "</a></td>"; //Aqui vai aparecer o nome da escola e se clicar ele redirecionar-me-à para a página de consultar essa escola em específico
							echo "<td><a href='ConsultarBlocos.php?pesquisa=" . $dados_computadores["idbloco"] . "&campo=idbloco' style='color: Black;'>" . $dados_computadores["nome_bloco"] . "</a></td>"; //Aqui vai aparecer o nome do bloco e se clicar ele redirecionar-me-à para a página de consultar esse bloco em específico
							echo "<td><a href='ConsultarSalas.php?pesquisa=" . $dados_computadores["idsala"] . "&campo=idsala' style='color: Black;'>" . $dados_computadores["nome_sala"] . "</a></td>"; //Aqui vai aparecer o nome da sala e se clicar ele redirecionar-me-à para a página de consultar essa sala em específico
							...
							for($j=21; $j<(count(str_split($dados_computadores["foto"]))); $j++)
							{
								$nome_foto.=$dados_computadores["foto"][$j]; //Faz a variável nome_foto receber o nome da fotografia
							}
						...
						echo "</tr>"; //Fim da linha
							...
							echo "<script>" .
									 "window.addEventListener('load', function()" .
									 "{" .
										 "if(!(document.getElementById('foto" . ($i+1) . "')==null))" . //Se a fotografia existe porque pode não existir no caso de estivar a editar os dados do computador então:
										 "{" .
											 ...
											 "{" .
												 ...
												 "document.getElementById('modalfoto').src='" . $dados_computadores["foto"] . "';" . //Faz o modal receber a fotografia
			...
			echo "</div>"; //Fim da zona da tabela
					...
					elseif($linhas_computadores==0) //Se não se retornar 0 linhas, significa que não existe nenhum computador preenchido então:
						{
										 ...
										 "{" .
											 "document.getElementById('zonaconsultar').innerHTML='' +" .
																								...
																								"'<select name=\'selectpesquisar\' class=\'selectpesquisar\' id=\'selectpesquisar\'>' +" .
																									...
																									"'<option value=\'tipo\'>Tipo de computador</option>' +" .
																									...
																									"'<option value=\'disco_rigido\'>Disco rígido</option>' +" .
																									"'<option value=\'disco_otico\'>Disco ótico</option>' +" .
																									"'<option value=\'placa_grafica\'>Placa gráfica</option>' +" .
																									"'<option value=\'placa_rede\'>Placa de rede</option>' +" .
																									"'<option value=\'fonte_alimentacao\'>Fonte de alimentação</option>' +" .
																									"'<option value=\'cooler\'>Cooler (Ventoinha)</option>' +" .
																								...
																								"'<select name=\'selectordenar\' class=\'selectordenar\' id=\'selectordenar\'>' +" .
																									...
																									"'<option value=\'tipo\'>Tipo de computador (Crescente)</option>' +" .
																									"'<option value=\'tipo DESC\'>Tipo de computador (Decrescente)</option>' +" .
																									...
																									"'<option value=\'so\'>Sistema operativo (Crescente)</option>' +" .
																									"'<option value=\'so DESC\'>Sistema operativo (Decrescente)</option>' +" .
																									"'<option value=\'ram\'>Memória RAM (Crescente)</option>' +" .
																									"'<option value=\'ram DESC\'>Memória RAM</option>' +" .
																									"'<option value=\'cpu\'>CPU (Processador) (Crescente)</option>' +" .
																									"'<option value=\'cpu DESC\'>CPU (Processador) (Decrescente)</option>' +" .
																									"'<option value=\'motherboard\'>Motherboard (Crescente)</option>' +" .
																									"'<option value=\'motherboard DESC\'>Motherboard (Decrescente)</option>' +" .
																									"'<option value=\'disco_rigido\'>Disco rígido (Crescente)</option>' +" .
																									"'<option value=\'disco_rigido DESC\'>Disco rígido (Decrescente)</option>' +" .
																									"'<option value=\'disco_otico\'>Disco ótico (Crescente)</option>' +" .
																									"'<option value=\'disco_otico DESC\'>Disco ótico (Decrescente)</option>' +" .
																									"'<option value=\'placa_grafica\'>Placa gráfica (Crescente)</option>' +" .
																									"'<option value=\'placa_grafica DESC\'>Placa gráfica (Decrescente)</option>' +" .
																									"'<option value=\'placa_rede\'>Placa de rede (Crescente)</option>' +" .
																									"'<option value=\'placa_rede DESC\'>Placa de rede (Decrescente)</option>' +" .
																									"'<option value=\'fonte_alimentacao\'>Fonte de alimentação (Crescente)</option>' +" .
																									"'<option value=\'fonte_alimentacao DESC\'>Fonte de alimentação (Decrescente)</option>' +" .
																									"'<option value=\'cooler\'>Cooler (Ventoinha) (Crescente)</option>' +" .
																									"'<option value=\'cooler DESC\'>Cooler (Ventoinha) (Decrescente)</option>' +" .
											 ...
											 "{" .
												 "location.href='ConsultarComputadores.php';" . //Volta para a página original
											 "});" .
										 "});" .
									 "</script>";
							...
							else //Se não:
							{
								if(isset($_SESSION["vista"])) //Se a variável vista existe
								{
									if($_SESSION["vista"]=="A") //Se a vista é de administrador então pode ser redirecionado para a página de inserir computadores:
									{
										echo "<script>" .
												 "window.addEventListener('load', function()" .
												 "{" .
													 ...
													 "Modal('computador', 'Sem computadores inseridos.');" . //Abre o Modal de que ainda não foi inserido nada
													 
													 "document.getElementById('zonabtnmodal').innerHTML='Será redirecionado à página para inserir computadores dentro de ' + segundos + ' segundos.';" . //Faz os botões desaparecerem e aparece a mensagem que vai redirecionar para a página de inserir computadores dentro de 5 segundos
													 ...
													 "{" .
														 "location.href='InserirComputadores.php';" . //Redireciona para a página de inserir computadores
													 "}, 6000);" .
													 ...
													 "{" .
														 ...
														 "{" .
															 "document.getElementById('zonabtnmodal').innerHTML='Será redirecionado à página para inserir computadores dentro de ' + segundos + ' segundo.';" .
															 "segundos--;" . //Diminui a variável segundos para ir acompanhando o tempo
														 "}" .
														 "else" .
														 "{" .
															 "document.getElementById('zonabtnmodal').innerHTML='Será redirecionado à página para inserir computadores dentro de ' + segundos + ' segundos.';" .
															 "segundos--;" . //Diminui a variável segundos para ir acompanhando o tempo
														 "}" .
													 "}, 1000);" .
												 "});" .
									...
									else //Se não, como o utilizador não está na vista de administrador então manda para a página inicial então:
									{
										echo "<script>" .
												 ...
												 "{" .
													 ...
													 "Modal('computador', 'Sem computadores inseridos, por favor informe o administrador.');" . //Abre o Modal de que ainda não foi inserido nada
						...
						elseif($linhas_computadores<0) //Se não se retornar menos que zero linhas, que existe algum erro inesperado na programação então:
							{
								$_SESSION["mensagemerro"]="Erro desconhecido. O número de linhas é inferior a 0, por favor informe o administrador."; //Passa a mensagem de erro
								header("Location: ConsultarComputadores.php"); //Recarrega a página passando o erro para abrir o modal de erro
							}
				...
				else //Se não, se ocorreu um erro:
				{
					if((isset($_GET["pesquisa"])) AND (isset($_GET["campo"]))) //Se foi feita uma pesquisa específica:
					{
						if(isset($_GET["ordenadopor"])) //Se foi feita uma ordenação então:
						{
							...
							if(($_GET["campo"]!="idequipamento") AND ($_GET["campo"]!="tipo") AND ($_GET["campo"]!="serial_number") AND ($_GET["campo"]!="posto") AND ($_GET["campo"]!="fabricante") AND ($_GET["campo"]!="modelo") AND ($_GET["campo"]!="so") AND ($_GET["campo"]!="ram") AND ($_GET["campo"]!="cpu") AND ($_GET["campo"]!="motherboard") AND ($_GET["campo"]!="disco_rigido") AND ($_GET["campo"]!="disco_otico") AND ($_GET["campo"]!="placa_grafica") AND ($_GET["campo"]!="placa_rede") AND ($_GET["campo"]!="fonte_alimentacao") AND ($_GET["campo"]!="cooler") AND ($_GET["campo"]!="prioridade") AND ($_GET["campo"]!="operacional") AND ($_GET["campo"]!="nome_escola") AND ($_GET["campo"]!="nome_bloco") AND ($_GET["campo"]!="nome_sala"))
							{
								//se a ordenação digitada não for nenhum válido
								if(($_GET["ordenadopor"]!="idequipamento") AND ($_GET["ordenadopor"]!="idequipamento DESC") AND ($_GET["ordenadopor"]!="tipo") AND ($_GET["ordenadopor"]!="tipo DESC") AND ($_GET["ordenadopor"]!="serial_number") AND ($_GET["ordenadopor"]!="serial_number DESC") AND ($_GET["ordenadopor"]!="posto") AND ($_GET["ordenadopor"]!="posto DESC") AND ($_GET["ordenadopor"]!="fabricante") AND ($_GET["ordenadopor"]!="fabricante DESC") AND ($_GET["ordenadopor"]!="modelo") AND ($_GET["ordenadopor"]!="modelo DESC") AND ($_GET["ordenadopor"]!="so") AND ($_GET["ordenadopor"]!="so DESC") AND ($_GET["ordenadopor"]!="ram") AND ($_GET["ordenadopor"]!="ram DESC") AND ($_GET["ordenadopor"]!="cpu") AND ($_GET["ordenadopor"]!="cpu DESC") AND ($_GET["ordenadopor"]!="motherboard") AND ($_GET["ordenadopor"]!="motherboard DESC") AND ($_GET["ordenadopor"]!="disco_rigido") AND ($_GET["ordenadopor"]!="disco_rigido DESC") AND ($_GET["ordenadopor"]!="disco_otico") AND ($_GET["ordenadopor"]!="disco_otico DESC") AND ($_GET["ordenadopor"]!="placa_grafica") AND ($_GET["ordenadopor"]!="placa_grafica DESC") AND ($_GET["ordenadopor"]!="placa_rede") AND ($_GET["ordenadopor"]!="placa_rede DESC") AND ($_GET["ordenadopor"]!="fonte_alimentacao") AND ($_GET["ordenadopor"]!="fonte_alimentacao DESC") AND ($_GET["ordenadopor"]!="cooler") AND ($_GET["ordenadopor"]!="cooler DESC") AND ($_GET["ordenadopor"]!="prioridade") AND ($_GET["ordenadopor"]!="prioridade DESC") AND ($_GET["ordenadopor"]!="operacional") AND ($_GET["ordenadopor"]!="operacional DESC") AND ($_GET["ordenadopor"]!="nome_escola") AND ($_GET["ordenadopor"]!="nome_escola DESC") AND ($_GET["ordenadopor"]!="nome_bloco") AND ($_GET["ordenadopor"]!="nome_bloco DESC") AND ($_GET["ordenadopor"]!="nome_sala") AND ($_GET["ordenadopor"]!="nome_sala DESC"))
								{
